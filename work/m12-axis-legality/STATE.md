...
- `pet-skill-release-mechanics.md` and research `STATE.md`: UI minimum gate is controlled Kibo alive, player in battle and `existPetBreakTarget=true`; click calls `firePetBreakSkill` through slot `JointStrikeSkill=601`. The exact authoritative generation of `existPetBreakTarget` remains open for this task.
...
- `existPetBreakTarget` remains unresolved. Verified `breakSkillList` plus `skillTag=15` identifies a Kibo joint action, but does not prove that the target trigger is true.
...
- Replaced self-signed Kibo authority with a compiler-owned pipeline:
  ...
  - compilation regenerates Kibo actions from the installed verified mechanics package and catalog, then materializes a deep-frozen WeakSet-authoritative registry;
  - registry/action identity binds package/catalog/mapping hashes, actor/Kibo/slot, action kind/skill, frame, per-slot sequence, source path/source source, controlled interval and switch-tail policy;
...
- Reworked Kibo scheduling around the controlled-actor timeline:
  - right-open `[startFrame,endFrame)` intervals are derived from accepted ordered switches; only the interval owner's equipped Kibo produces new normal/active casts;
  - per-slot cooldown, shared group lock and sequence persist across switch-away/switch-back; no third-slot or off-field recast is generated;
  - exact switch frame belongs to the new owner after the switch source path; timeline/interval/rejections, source paths and registry hashes are projected into canonical input/data/trace/build identity;
  - tag `0` is generated from closed static evidence. Nonzero trigger tags become hash-bound `kibo-auto-cast-trigger-unresolved` exclusions, including the zero-generated-action case, and reject all three formal objectives without fabricating damage.
...
| Owner/target/scenario            | Existing preflight plus foreground/joint wiring | Controlled owner, equipped Kibo, source slot, target/range and passive-boss exclusions fail before score                                      | `existPetBreakTarget` generation remains unresolved                                                            |
...
| Kibo derivation authority        | Completed in R3                                 | Compiler generation -> installed package/catalog mapping -> immutable registry -> runtime exact match; clones/raw declarations fail closed    | New autonomous trigger mappings must be added through source data, not embedded action fields                  |
| Controlled Kibo activity         | Completed for closed tag-0 autos                | Right-open controlled intervals, ordered switch transitions, per-slot cooldown/sequence and owner/slot hashes                                 | Nonzero trigger tags stay `kibo-auto-cast-trigger-unresolved`; background energy recovery remains independent  |
