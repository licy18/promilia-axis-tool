...
- `m12-enemy-settlement-runtime-v2` records the current runtime packet order and the product-approved formal-score basis. `cycle-dps-with-toughness` and `fastest-kill` now return formal scores without the internal timing override, while client parity remains explicitly pending and future contradictory evidence requires contract replacement plus score recomputation.
...
- At OPT-T2 entry, the gate was fail closed: `m12-enemy-settlement-runtime-v1` was `formalReady=false` with `machine-axis-enemy-settlement-client-order-open`. The later runtime-baseline decision below supersedes scoring readiness without rewriting this historical evidence state.
...
## Runtime-Baseline Formal Score Decision (2026-08-09)

- Product decision: publish formal scores using the current headless settlement core now, then revise through a new contract if controlled client evidence differs. No client capture is claimed.
- Contract bumped to `m12-enemy-settlement-runtime-v2` (`contractHash=2e3095db4b8c9232`). Current authority is `formal-for-current-runtime-contract` with `formalReady=true`; evidence remains `clientParityReady=false` and emits `machine-axis-enemy-settlement-client-parity-pending`.
- Evidence wording was regenerated only to bind the v2 runtime identity; findings, `formalReady=false` evidence conclusion, and all four capture leaves remain unchanged. Source artifact: 18,807 bytes / `7d162ce61cc7c3dec1dafb48e590b61be45e27ad29e0bcf4d8b39e3b87b2f951`; generated report: 19,799 bytes / `4999f4201c92f7e281bd4dbc3bfb8e84fb8573c68c688eaf295ee3fdcec13bfb`; semantic `reportHash=a54468f259bb2e4d1ad0e8981048a72855add385f2417a5488d8f135a991bd5d`.
- Frozen runtime basis: breaking packet reads pre-break; toughness and break state settle before HP mutation; later same-frame packets use canonical source sequence; weakness advances on fixed 100ms ticks; break end is right-open with state tick before hit. Formal scenario path is selected as `local-controlled`, without claiming the fixed-tick clock matches native per-update deltaTime.
- `cycle-dps-with-toughness` emits formal DPS only after strict two-loop closure. `fastest-kill` emits formal TTK only for a real first lethal cursor. Post-lethal tail settlement is diagnostic-only and cannot change the first-lethal score; runtime truncation remains implemented.
- Future capture preflight output now separates `formalScoringPolicy` (v2 is score-ready, preflight does not evaluate a score) from `clientParityGate` (capture still pending with four open observations), avoiding the old ambiguous `formalGate` snapshot.
- Formal admission no longer emits `machine-axis-enemy-settlement-client-order-open`. Legacy objectives, unresolved enemy profiles, invalid scenario inputs, failed cycle closure, and qualification stage gates remain hard failures. M12-C stays locked and no formal optimization was run.
- Pending evidence is unchanged: same-frame cross-packet visibility, break-end client-frame ordering, native lethal-tail disposition, and path observation remain uncaptured. Score-sensitive evidence differences trigger a new contract version and invalidation/recomputation of affected scores.
- Focused failure-to-pass: the first post-change run had 73/74 pass because the no-toughness committed cycle report received an unnecessary `formalScorePolicy:null`; the field is now emitted only for toughness-enabled cycles, preserving the no-toughness golden. Follow-up focused contract/kill/qualification passed 37/37 and targeted cycle formal-score/golden tests passed 2/2.
- PASS verification: evidence/contract/capture-manifest/host 25/25; full verified runtime + cycle + kill + objective/profile/settlement 93/93; search/CLI/report/Workbench 72/72; scenario/qualification/capture-normalizer/Workbench capture 59/59. Evidence and capture-manifest assert-clean pass; scenario policy remains `967b0667f315db5b`; qualification remains `m12cLocked=true`, catalog `b26e4d9379d9a95d`; Workbench-data audit passes.
- PASS build/audit: Machine Axis CLI build and production build (1,886 modules); production-imports assert-clean twice with `reportMatches=true/outputChanged=false`, unchanged report SHA/mtime/Git status; Node/Python syntax, changed-file ESLint, Prettier, and `git diff --check` pass.
- `audit:verified-combat` was run once and completed with pre-existing golden drift for five M10 reports (`101003/101010/103002/109001` golden traces plus `101003` ultimate controlled-buff-switch). This batch changes no `src/simulation`, verified package/data, generator, or M10 report input, so unrelated goldens were not regenerated or added to this commit.

## Central Semantic Integration (2026-08-09)

- The five-commit optimizer chain was replayed in order onto central role baseline `ee5c4cf817521165186f438f274893c91dfb1280`; the resulting mechanism head is `294d1085`. Shared runtime/compiler files were merged semantically, while central character package/profile/acceptance state was preserved.
- Windows checkout exposed a real byte-identity portability defect in the static evidence artifact: the working file became CRLF while the committed evidence hash assumed LF. Central now fixes `scripts/machine-axis/evidence/*.json` to `eol=lf`; two consecutive evidence assert-clean runs reproduce `reportHash=a54468f259bb2e4d1ad0e8981048a72855add385f2417a5488d8f135a991bd5d` without writes.
- Current central qualification is `9/43/62/137/12`, `blockingUniqueGapCount=7`, and `m12cLocked=true`. Runtime-v2 formal scoring is available under the versioned headless contract, but no formal search ran and `clientParityReady=false` remains explicit.
- Central verification passed settlement/runtime/cycle/kill/capture/qualification tests after deterministic lock regeneration, search/CLI/Workbench/Machine Axis regression, both builds, scenario/qualification/Workbench audits, and two read-only production-import checks with stable SHA-256 and mtime.
