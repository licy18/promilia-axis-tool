...
- ultimate semantic (product ruling `M12-B3-STARBORN-MARK-R2`, superseding the stale `+1` oracle and the R1 `+2` reading): add one layer to each already-present elemental tuning mark, cap five;
...
- The raw client wrapper proves the `count > 0` eligibility condition. The current `ultimate +1` value comes from product ruling `M12-B3-STARBORN-MARK-R2` (2026-08-10), superseding the stale `skills.json`/golden `+1` oracle as well as the R1 `+2` reading; the star-carry value remains `+1`.
...
- Ultimate tuning-mark behavior is limited to existing elemental marks and adds one with a per-type cap of five. Machine projections retain condition-gate and acquired mark component sources; no missing mark is invented.
