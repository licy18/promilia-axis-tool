...
- **E3（E3a 已完成，2026-08-05）**：composite 家族多触发器单叶 —— 生成器支持多触发器编译（`triggers` 数组 + 共享单叶校验 + 每触发器条件/目标），运行时按 triggerDefinitions 迭代并在触发条件中支持元素公式（conditionType 10000 → `element-formula-layer-gt`，含印记层数门控）。已关闭 **10008 可以吃吗、10071 羽夜的思念、10146 新生**（runtime-applied 58/16，阻断 365，not-implemented 344）。E3a 验证：灵魂运行时 109/109（新增 10146 多触发、10071 印记 950/850 门控、10008 目标元素 fail-closed 用例）、资格套件 13/76、Machine Axis 12/160、applied-source-bindings 审计扩展（10000 条件来源识别）+ 全审计通过。剩余 composite：多叶组（10076/10095/10121/10122/10132/10196）、事件/标签缺口（10063 event32+damage、10011 event25+wrapper、10198 tag5/14）。
