...
- **E3（E3a/E3b 已完成，2026-08-05）**：composite 家族 —— E3a 多触发器单叶：生成器多触发器编译（`triggers` + 共享单叶 + 每触发器条件/目标），运行时按 triggerDefinitions 迭代 + 触发条件元素公式（10000 → `element-formula-layer-gt` 印记层数门控）。已关闭 **10008/10071/10146**。E3b 多叶：生成器 `effectLeaves` 逐叶编译（共享叶编译器 `compileSoulEffectLeaf`，逐叶公式/桶/时长/栈/星级校验）+ 触发器→叶可达集合的 target 解析 + 事件元素条件（type 8/13）已知元素集校验；运行时按叶逐条发射命令（set-skill 保持原单命令语义）。已关闭 **10121 折纸寄情、10122 花之舞、10170 攀高的幼崽、10196 热火朝天**（runtime-applied 62/12，阻断 357，not-implemented 336）。E3a/E3b 验证：灵魂运行时 112/112、资格套件 13/76、Machine Axis 12/160、审计通过。剩余 composite：10076（元素类型条件 8/307/32 未闭合）、10095（叶 19003206 缺星级数值源）、10132（BeforeDamage 空条件证据）；事件/标签缺口：10063（event32+damage）、10011（event25+wrapper）、10198（tag5/14）。
