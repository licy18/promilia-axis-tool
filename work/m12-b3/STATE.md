...
- Current known blockers include `24/62` soul-essence effect skills and `4/12` set skills classified as `dynamic-unapplied`; `38/62` source-closed soul effects and `8/12` set skills are runtime-integrated but still lack complete qualification.
...
- Not implemented: `368` unique blockers, including 24 unresolved soul-essence dynamic skills, 4 remaining four-piece set skills, remaining runtime/visual qualification, and one missing STARBORN static actor profile. DNA contributes zero source or acceptance gaps.
