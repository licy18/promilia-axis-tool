...
- clean `f11ab4c12a435dd4ed8e5d89ce963fa3d44533e3` 上的第五次 release 没有形成业务结论：承载前台命令在约 30 分钟工具层超时后被外部终止，遗留 preview 子进程只把日志推进到 62/64，未回到 release 编排器完成 restoration、postflight、Gate V2 原子记录或 admission。wrapper exit `1` 不得解释为产品 FAIL，`latest-release-verify.json` 仍停留在上一轮 `4ec04079` 的真实 FAIL。确认全部子进程退出后，唯一 tracked 漂移为 100 个直接 `reports/*.png`，已按 `release-runner-output-v1` 边界精确恢复；无其他 tracked 漂移，全部 untracked evidence、污染文件和 stash 未触碰。下一轮须从新的 clean docs-only HEAD 由独立后台进程完整执行，直到 Gate V2 自身落盘。
...
- 第三次 release 在 clean `483cf1530fc156023b2754894c012329c4dff50d` 上于独立 `binding` gate 诚实失败，record `da56914c0c2e398a8ac8795301f645afa291af46007d61f913e1e320bad58dfe`：重绑后的 qualification 已通过，但 `reports/m12/m12-b3-binding-matrix.{json,md}` 仍是旧 subject 的派生镜像。两件已用权威生成器刷新并 `22/22` clean，binding + outer build pool 回归 2 files / 19 tests PASS；第三次 FAIL 不产生 admission，仍须提交后从新 clean HEAD 完整建权。
- 第四次 release 在 clean `4ec040794b6f185df2a6dea7253a2704d899c6be` 上通过全部独立 gates 后，于 full Vitest 仅剩 1/2070：`m12-b3-c-dynamic-loadout-effect-acceptance.json` 的六个 qualification hash 镜像仍指向更早基线。release record `d9793c1f9b7b5061a6fc0f0a0af05cced0b9981d330c38b810bf08c2a8002f9e` 为 `FAIL (executed)`；封套六字段已同步当前权威 `f760/eaf6/bb3f/3c62/a525/9c68` 全值，所有 10 个引用该封套的 evidence tests 35/35 PASS。旧 FAIL 不得产生准入。
