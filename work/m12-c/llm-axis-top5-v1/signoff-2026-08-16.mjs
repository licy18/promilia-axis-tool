...
if (!EVIDENCE_COMMIT || !/^[0-9a-f]{40}$/.test(EVIDENCE_COMMIT)) {
  console.error(
    'M12C_SIGNOFF_EVIDENCE_COMMIT required (40-hex evidence commit)'
  );
  process.exit(1);
}
...
if (stage2) {
  for (const ownerId of owners) {
    ...
    const knownHashes = new Set(
      existingSuperseded.map(e => e.screenshotSha256 ?? '').filter(Boolean)
    );
    for (const evidence of priorEvidence) {
      if (!knownHashes.has(evidence.screenshotSha256 ?? ''))
        existingSuperseded.push({ ...evidence });
    }
    ...
    const expectation =
      manifest.evidence?.productVisualAcceptance?.bindingExpectation ?? null;
    if (!expectation) {
    ...
    console.log(
      `${ownerId} stage2: subject=${expectation.qualificationSubjectHash} recordIdentity=${expectation.recordIdentity.slice(0, 44)}...`
    );
  }
  console.log(
    'STAGE2 DONE — 提交 record + recipe 后运行 --stage3 回填 record SHA'
  );
}
...
if (stage3) {
  ...
  for (const ownerId of owners) {
    ...
    try {
      ...
      console.log(
        `${ownerId} FAIL: record not found in ${recordCommit.slice(0, 8)}:${recordRelative}`
      );
      continue;
    }
