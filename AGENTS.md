...
- 工作台伤害投影已建立公式分层雏形：`formulaBreakdown` 中当前攻击和动作形态倍率是已应用层，敌人防御/抗性/暴击/增伤仍是 `applied: false` 的占位层。
...
当前已完成阶段 1-3 的最小闭环、阶段 4 工作台主链路、阶段 5-1 至 5-8D 的真实数据/数值/动作形态、直接动作库、60fps 帧时间轴、公式分层雏形和战斗公式证据索引。旧 Vue 原型可运行，但不再作为最终架构地基；下一步推进阶段 5-8E，继续追踪 skill asset / effect node，或先把公式证据索引接入公式分层 source 说明。
