...
当前已完成阶段 1-3 的最小闭环、阶段 4 工作台主链路、阶段 5-1 至 5-8AJ 的真实数据/数值/动作形态、直接动作库、60fps 帧时间轴、公式分层雏形、战斗公式证据索引、公式 source 接入、skill asset/effect node 候选索引、每动作三值结果契约、`skill_control` 效果轨道候选分类、本地行为链解引用、elementBaseDatas 资源映射归属、行为脚本类型候选、IL2CPP element 类型目录、外部 element 对象本体解析、`TDamageElementParams` 三值字段候选映射、Workbench 三值来源展示、`valueParam` / `formulaParamValues` 槽位关系诊断、未应用公式槽位候选展示、`function_1/function_2 -> element_formula` 候选索引、动作级 `formulaFunctionSummary` 展示、`formulaCandidatePreview` 未应用预览、`combinationPreviews` 简单组合诊断矩阵、`formulaCandidatePatternSummary` 跨动作差异模式摘要、`skillControlBehaviorCorrelations` 技能级行为节点关联摘要、按 lane 保留的 HP 行为链样本、动作形态级行为绑定候选、`stateTimingEvidence` 状态/时序控制证据、EventBridge 目标技能 skill_control 摘要、普攻连段链递归索引、`normalAttackHitChainCandidate` 普通攻击 1-5 段 / 每 hit 候选、每段 hit 的 `behaviorList -> elementBaseDatas -> TDamageElementParams` 三值字段候选、`actionResultTimeline[].hitCandidates[]` 未应用 per-hit 三值增量预览、顶层 `candidateValueSeries` 候选三曲线聚合、`candidateValueSeries.chart` 60fps 多曲线图表层、基于 EventBridge 的普攻连段绝对帧候选、主时间轴候选三值 marker、主时间轴候选多曲线轨/按帧提示、候选曲线显隐、选中帧来源摘要、chart point 级详情下钻、选中帧范围过滤、per-element 原始字段详情、per-element 公式函数/等级槽位摘要和 actor/action/series 组合过滤入口。当前默认普攻动作输出 5 条 hit 候选、15 个曲线点，本地帧 `12 / 6 / 12 / 7 / 4f`，EventBridge 绝对帧 `0s12f / 0s22f / 1s3f / 2s3f / 3s4f`，连段桥接 `4/4`，主时间轴默认显示 3 条候选曲线、15 个候选 marker 和 5 个按帧 hotspot；首帧 per-element 详情可区分 `109001306` 的 HP `1,000/1,800/2,500`、函数 `f1:G/10000 / f2:self.ATK[0]*A/10000`、槽 `A覆盖1,600-3,360 / G直连10,000`、韧性 `7,000`、能量 `2,700`，以及 `109001081` 的 HP `1,000/1,900/2,500`、同样函数/槽位、韧性 `7,000`、能量 `2,700`；组件级 fixture 已验证双 actor / 双 action / 三 series 的组合过滤。下一步推进阶段 5-8AK，把 per-element 详情整理成可横向比较的候选详情模型/tooltip，并继续追 `DamageElement` function 组合顺序、等级覆盖应用点和每 hit 倍率分配。
