...
### M10-A 单角色战斗解析流水线已实现，等待产品验收（2026-07-26）

现可按 `ownerId` 用同一命令生成可达战斗图、版本化角色 profile、verified runtime 输出绑定、缺口/采样账本和 golden trace；小玉金标准固定为 10 个公开动作、21 个执行形态、20 个可达 control、86 条窗口和 108 个 hit，单角色及全量审计均为零漂移，生产运行时与 UI 未新增角色 ID 特判。完整单测 133 文件/775 条和小玉 production preview 3/3 通过；4 条投射物实测需求与 370 条 raw 静态证据缺口继续保留。全历史 production preview 的 20 条旧按需面板定位失败及总 JS gzip `746,688B` 作为独立风险记录，本阶段不修旧 E2E、不压包，也不自动进入 M10-B。下一步经产品验收后按红宝石、寒悠悠及 coverage manifest 顺序逐角色推进。
