...
### M12-B2-R1 可持续循环 DPS 整改完成，等待产品复验（2026-08-01）

Machine Axis 的 `AzPrMachineAxisCycleDps` v1 与 CLI `cycle` 继续由唯一 canonical core 计算显式半开循环区间。R1 将无限 HP 语义前移到普通、调谐/超限和真实伤害的公式输入，初始敌方 HP 不再逐 hit 封顶伤害，护盾合同保持有效；奇波被动内置 CD、累计触发次数与有限触发寿命现在进入 canonical trace 和循环边界状态，按相对剩余时间/次数比较，不能再由固定两轮伤害相同伪装为稳态。权威红宝石 A1-A3 示例数值保持每轮 `22.44996643`、4 hit、`cycleDps=4.48999329`，驮驮龙 15 秒内置 CD 的短循环已成为明确负例。当前停在 M12-B2-R1 产品复验点，M12-C 未启动。
