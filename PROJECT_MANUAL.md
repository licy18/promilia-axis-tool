...
### M2 三人队配置可用性滚动缺陷整改完成，等待重新验收（2026-07-14）

时间轴身份区已成为基础换人与装配的主入口：三名角色可直接点击头像换人，每个角色固定显示奇波、武器、上装、下装、耳环、戒指和灵子槽位，奇波子轨与敌人身份也可直接打开同一套可视选择器。重新验收发现长列表未形成面板内滚动区且滚轮会带动后台页面；现已改为固定高度的内部滚动区、常驻滚动轨和模态后台滚动锁，桌面及窄屏均能滚动奇波、装备、灵子和敌人候选而不移动工作台。既有团队、配置、敌人更新合同、六条能量轴和五载体回放保持一致，培养效果仍为 `unapplied`。

真实默认/空方案装配路径、撤销/重做、拖入动作与保存回载已纳入浏览器守门，M2 路径现直接验证长列表 `scrollTop` 增长、可见滑块同步和后台页面位置不变。93 个测试文件、508 条单元测试，41/41 production preview 和 40/40 Workbench 主流程 E2E 全部通过；生产数据与 applied-source 审计通过，总 JavaScript gzip 为 719,622B。新增验收截图为 `m2-loadout-picker-scroll-desktop.png` 与 `m2-loadout-picker-scroll-narrow.png`。下一步停止实现并等待 M2 重新验收，P3 非 fixture 真实采样继续暂停。
