...
### M12-B3-A-R1 优化资格基线等待产品复验（2026-08-01）
...
B3-A-R1 已纠正角色培养来源边界：`talent_rank` 的第 7 层节点可生效，但属性只累计已完成的 1..6 层；`hero_rank` 独立建模为等级上限突破合法性，严格校验 level/rankLevelLimit 组合，其属性和技能解锁在缺少客户端应用证据时保持 unapplied。80 级小玉 naked actor 已逐属性对齐既有面板，修正后的角色属性继续传播到奇波继承与最终伤害。formal Machine Axis 的准入不再只看当前所选对象，而由 11/43/62/137/12 全分母、12 个套装技能、角色-奇波/灵子/装备绑定、装备部位及 hash 一致性共同派生；部分对象先绿、任一 set/binding 红、计数少 1 或 hash 漂移都在 project/search 前结构化拒绝。8 个 runtime-integrated 灵子统一覆盖 Before/After、stack/refresh、三类动作、持续/到期、错误动作抑制与同帧结算，AfterSkill 不反向增益自己的末帧 hit。当前 source/roster/manifests/ledger/catalog/binding hash 为 `95cac6744423e44b / e574c0d8d5c9f335 / 41d8d2857ee6c699 / f31dfaa6bed204c3 / 9a7300aa8825aba6 / d21335a454947b3c`；五类准入仍为空、727 个阻断不变，当前停在 B3-A-R1 产品复验点，M12-C 继续锁定。
