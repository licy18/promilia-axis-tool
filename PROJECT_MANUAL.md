...
B3-A 已生成带来源快照的 roster、逐对象 manifest、缺口账本、绑定矩阵和 canonical catalog。严格培养 profile 可表达角色、奇波、灵子及五部位装备培养值，10 级奇波四维解析为 `120`，羁绊 1 级解析为 `9%` 且拒绝 0；原始值与解析值共同进入 canonical hash。已闭合的静态培养维度会在建 project 前投影到唯一属性编译器：角色星赐此前层完整节点、当前层显式所选节点及六档临阶属性按来源独立汇总，装备同调使用 `game.json` 的四参数公式与分段 `ceil`。DNA 按产品决定统一为当前版本 `not-applicable`：生成器不读取因子或联动源，不产生 source/acceptance gap，Schema 只接受空值并在 canonical 输出显式保留。灵子生成层已闭包 62/62 个效果控制与 282 条资源引用；通用算子仅应用 8 个来源闭合效果，54 个继续保留精确缺口，真实属性 222 的后续削韧变化已证明运行时传播。当前 source/roster/manifests/ledger/catalog/binding hash 为 `95cac6744423e44b / e574c0d8d5c9f335 / 9f827448f4cfa80e / 1d42563740f73b0e / 53d0a03f98e6777e / c99404afb388f461`；五类准入集合仍为空，formal Machine Axis 在 project 生成前硬拒绝，research 轴不受影响。这只是可信基线，不代表任何对象已具备优化资格。
