...
### M12-B2 可持续循环 DPS 实现完成，等待产品验收（2026-08-01）

Machine Axis 新增版本化 `AzPrMachineAxisCycleDps` v1 与 CLI `cycle`，由唯一 canonical core 强制无限 HP、关闭韧性/击破/死亡截断，并按实际 hit 帧统计显式半开循环区间。闭环同时检查角色与奇波能量、特殊资源、队伍印记、主控、CD、持续效果、待结算事件和最终动作形态；同一语义循环连续重放两轮，第二轮不可被资源、CD、条件、重叠或派生规则阻断。调谐印记的闭环按资源层数不劣、共享衰减余量不退化及独立就绪等待不增加判断，层数增加不会掩盖 5 秒就绪门。权威 5 秒红宝石 A1-A3 示例两轮均为 `22.44996643` 伤害、4 hit，`cycleDps=4.48999329`；零距离投射物仍诚实保留为场景假设和开放证据。当前停在 M12-B2 产品验收点，M12-C 未启动。
