...
### M12-B-R1 完成，循环 DPS 为下一优先级（2026-08-01）

M12-B-R1 已完成重复动作、真实节点状态、事件等待、评分窗口、暴击 seed 安全门和跨队伍 Top-N 整改，当前等待产品复验。下一优先级为可持续循环 DPS：默认无限 HP、无韧性，以显式半开循环区间内总伤除以区间时长；循环结束的 SP、奇波能量、弹药、特殊资源、队伍印记等不得少于开始，CD、充能、内部 CD、形态与条件必须允许下一轮无阻断重放。至少连续执行两轮验证闭环，报告循环起止、伤害、DPS、状态差、CD 证明、贡献、hash 和暴击 seed。M12-C 继续锁定。
