...
B3-A 已生成带来源快照的 roster、逐对象 manifest、缺口账本、绑定矩阵和 canonical catalog。严格培养 profile 可表达角色、奇波、灵子及五部位装备培养值，10 级奇波四维解析为 `120`，羁绊 1 级解析为 `9%` 且拒绝 0；原始值与解析值共同进入 canonical hash。已闭合的静态培养维度会在建 project 前投影到唯一属性编译器，装备同调使用 `game.json` 的四参数公式与分段 `ceil`，旧轴不物化这些派生字段因而保持原 hash。星赐节点/临阶、DNA、灵子升星动态与装备实例档仍明确未应用。当前 qualification catalog 的五类准入集合均为空，formal Machine Axis 在 project 生成前硬拒绝，research 轴不受影响；这只是可信基线，不代表任何对象已具备优化资格。
