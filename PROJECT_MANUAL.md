...
### M9-R2 全角色派生控制与零时长换人事件（2026-07-22，进行中）
...
M9-R2A-R2C 已完成全角色派生控制审计、首条可用选择链和零时长切人事件：20 名角色的 136 个候选 control 均有来源合同，63 个多变体 control 无静默遗漏；已确认的蓄力档位由用户选择，资源/状态分支仍由输入帧运行时决定。切人现在只在精确帧改变受控角色，不占普通动作区间、不生成三值或状态，同帧冲突确定拒绝；旧 600ms 事件迁移后保持原始起始帧及后续动作绝对位置。下一步只接入真实入场/退场星携技绑定与子动作回放。
