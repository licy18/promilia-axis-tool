...
B3-A 已生成带来源快照的 roster、逐对象 manifest、缺口账本、绑定矩阵和 canonical catalog。严格培养 profile 可表达角色、奇波、灵子及五部位装备培养值，10 级奇波四维解析为 `120`，羁绊 1 级解析为 `9%` 且拒绝 0；原始值与解析值共同进入 canonical hash。当前 qualification catalog 的五类准入集合均为空，formal Machine Axis 在 project 生成前硬拒绝，research 轴不受影响；这只是可信基线，不代表任何对象已具备优化资格。
