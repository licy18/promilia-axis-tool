...
### M12-B3-C7-R2 等待产品复验（2026-08-02）
...
B3-A-R1 已纠正角色培养来源边界并在 `f902de10c42c2c4dc750be2316fabe3bc026f8cc` 通过产品验收。B3-B-R1 进一步确认 `HeroData.Populate(HeroAttrInfo)` 与 `RefreshAttributes` 的调用链不能排除服务端 `AttrModuleInfo` 已含突破属性，因此撤销 `hero_rank.attribute` 的静态累加并把技能表声明、实际可用性和效果运行时分开；该阶段已在 `f846161c4a71bbc2de2b5bed3f598f03344fc692` 通过产品验收。普通/缘星实例、固定培养与 DNA=`[]` 合同保持不变。B3-C-R1 已在 `2804f201ac2a6ea4eebc1339703a9d40c0aba5a5` 通过产品验收，C2-R1/C3/C4-R1/C5/C6 分别在 `ba3422c722f8640857fa8fd9d19040e755c8484a`、`e05b10fd27a6c723a773a7680169a9180031c48e`、`8bf0b3a77538102c1995dd5943e4a7275664ca0a`、`5c713e55ffdc46e5e75e9a4a4dcc1d0366201be0`、`5c5a33a21cce05fbf090af72c783bad475b55656` 通过。C7 从真实切入、直接/周期治疗和直接护盾 settlement 发布稳定非伤害事务；被复验打回的 `5647f1a74d74cb63b763ed06367ac8198084cab1` 在预投影中保留成本却遗漏此前资源收入。C7-R1 基线 `8760b14b448ff22290895736a6f7b19c077fe589` 已补齐确定性资源投影，但 full runtime 又把 landed hit 回能错误绑定到伤害公式 ready。C7-R2 依据 `AliveElementSystem.ExecuteDamageElement -> DamageElement.RecoverSP` 独立调用，把回能资格统一为动作执行、占轴、恢复源字段、landed transaction identity 与 interval；伤害输入缺口继续输出 unresolved，资源事务仍可独立结算，投影继续不计算伤害、削韧或暴击。`10048/10175` 的事件目标合同不变；`10169/10176` 仍因护盾刷新/替换与 `combineType=5` 证据不足而不应用。当前分母仍为 11/43/62/137/12，灵子为 25 applied / 37 unapplied，阻断账本为 420 条（402 not-implemented、18 evidence-insufficient），五类准入仍为空。整个 B3 尚未完成，M12-C 继续锁定。
