...
### 2026-07-09：UI 主流程可见闭环 - Workbench End-to-End Loop

本阶段属于：UI 主流程可见闭环。

主流程缺口检查结论：

- `WorkbenchFlowPanel` 已提供“查看运行结果 / 编辑结果动作 / 回到刷新结果”的主路径入口。
- 资源曲线点已能选中对应运行结果，并同步日志行、三值详情、时间轴动作和状态曲线点。
- 日志行已能选中对应运行结果，并同步三值详情面板。
- 三值详情已能把用户带回动作编辑；动作修改后已有刷新结果回看入口。
- 当前最需要补的是一条端到端测试，把这些已存在的可见能力固定为同一条用户路径，避免后续重构时只保证局部面板测试通过。

用户现在能完成的实际流程：

1. 在工作台点击“查看运行结果”。
2. 在资源曲线中点击运行点，看到对应日志行和三值详情同步选中。
3. 点击日志行查看同一个三值详情。
4. 在三值详情中点击定位/编辑动作，回到动作时间字段。
5. 修改动作开始时间后，看到刷新后的运行结果。
6. 点击“回到刷新结果”，回到新的三值详情和资源曲线定位。

当前验证事实：

- 新增 Workbench 端到端测试 `supports the visible workbench loop across curve, log, detail, edit, and refreshed result`，完整覆盖上述可见路径。
- 本阶段不新增公式推断、不调整三值结果、不扩展局部提示或文案状态。
...
- 如果继续 UI 主流程，应从可见编辑体验入手，例如动作编辑表单的完整性、批量操作后的可见反馈、结果区与编辑区的布局效率；如果转到底层，则进入运行时层，继续稳固 `simLog`、`stateCurves`、资源曲线和 summary 的标准合同消费。

### 2026-07-09：UI 主流程可见编辑体验 - 60fps Frame Timing Controls

本阶段属于：UI 主流程。

完成的可用能力：

- Workbench 属性面板新增“开始帧 / 持续帧”编辑入口，用户可以直接按 60fps 的 1 帧粒度调整动作开始点和动作长度。
- 帧输入继续写回现有 `startMs` / `durationMs` 字段，并复用已有动作编辑、刷新结果和回到结果点链路；没有改动三值公式、倍率、运行时数值或 evidence 逻辑。
- 上一阶段的可见闭环现在可以走“详情定位动作 -> 按帧修改开始帧 -> 回到刷新后的运行结果”，更贴近 Endaxis 式排轴操作。

当前验证事实：

- Workbench 端到端闭环测试改为通过“开始帧”修改动作后回看刷新结果。
- 新增 Workbench 测试覆盖 60fps 帧控件把 `30` 帧映射为 `500ms`、`45` 持续帧映射为 `750ms` 并保存到草稿。
...
- 继续 UI 主流程能力块：优先改善 Workbench 主界面的编辑密度和结果区布局，让动作编辑、资源曲线、日志和三值详情在同一条工作流里更少切换、更容易定位；暂不进入公式考据或真实倍率补全。

### 2026-07-09：UI 主流程布局效率 - Runtime Detail First Inspector

本阶段属于：UI 主流程。

完成的可用能力：

- Workbench 右侧检查区在动作编辑状态下继续保持“属性”优先，便于直接改动作。
- 当用户进入运行结果查看或刷新结果查看状态时，右侧检查区会把“三值详情”提升到第一位，让用户从资源曲线或日志选中结果后能马上看到 HP / 韧性 / 能量明细与“编辑结果动作”入口。
- 本阶段只调整主流程布局顺序，不新增公式推断、不调整三值结果、不改变保存 schema。

当前验证事实：

- Workbench 页面测试覆盖动作编辑态右栏属性优先、运行查看态右栏三值详情优先。
- 既有可见闭环、帧级编辑、运行结果回看测试继续通过。
...
- 继续 UI 主流程能力块：优先把资源曲线、日志和三值详情之间的结果定位体验继续做得更像一个完整工作台，例如结果详情的紧凑化、结果区垂直空间分配或贡献拆分入口整理；继续避免进入公式考据或真实倍率补全。

### 2026-07-09：UI 主流程结果详情 - Contribution Summary

本阶段属于：UI 主流程。

完成的可用能力：

- 三值详情面板新增“本点贡献”紧凑摘要，用户从资源曲线或日志选中一个运行结果后，可以先看到当前结果主要贡献到 HP / 韧性 / 自身能量中的哪一项。
- 原有三条贡献明细继续保留在摘要下方，作为完整拆分；摘要只消费既有 `runtimeSelectedDetail.contributionRows`，不新增公式、不改三值结果。
- 这让“资源曲线/日志选点 -> 右栏三值详情 -> 看贡献拆分 -> 回到动作编辑”的主流程更短。

当前验证事实：

- Workbench 页面测试覆盖从资源曲线选中运行点后，三值详情同步显示贡献摘要，并确认活跃贡献轨道为 HP。
...
- 继续 UI 主流程能力块：优先整理结果区垂直空间和日志/曲线/详情之间的切换效率，让当前已有曲线、日志、详情、贡献拆分更像一个统一结果检查台。

### 2026-07-09：UI 主流程结果详情 - Detail Navigation

本阶段属于：UI 主流程。

完成的可用能力：

- 三值详情面板新增上一条/下一条运行结果导航；当模拟结果超过 1 条时，用户可以直接在右侧详情里切换结果点。
- 导航复用已有 `flowModel.runtimeNavigation` 和主流程 `select-runtime-state-point` action，切换后继续同步资源曲线、日志行、时间轴动作和详情状态。
- 本阶段只把既有导航能力落到详情面板，不新增公式、不改运行时输出、不改变保存 schema。

当前验证事实：

- Workbench 页面测试覆盖从三值详情面板切换上一条/下一条运行结果，并确认主流程选中动作和详情状态点同步更新。
...
- 继续 UI 主流程能力块：优先把结果检查台的空间组织继续收紧，例如让日志与详情的高频信息更少重复、让曲线选择和详情导航在同一检查路径中更连贯。

### 2026-07-09：UI 主流程结果检查台 - Compact Log Detail

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 当用户从模拟日志或资源曲线选中一个运行结果，且右侧三值详情已同步时，日志详情区改为紧凑定位摘要，只保留动作与状态点。
- 完整的三值、贡献、来源和适配器信息继续由右侧三值详情承载，日志面板不再重复展开同一批高频字段。
- 未进入右侧详情同步时，日志详情仍保留完整 fallback 明细，便于兜底查看。

当前验证事实：

- Workbench 页面测试覆盖初始 fallback 日志详情为完整布局、选中日志后同步右侧三值详情，并确认日志详情切换为紧凑布局。
...
- 继续 UI 主流程能力块：优先把曲线点、日志行和右侧详情的结果定位路径做成更完整的工作台体验，例如进一步检查端到端编辑后刷新结果的入口和空间分配；仍不进入公式考据或真实倍率补全。

### 2026-07-09：UI 主流程编辑反馈 - Refresh Result Primary Action

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户从结果详情进入动作编辑并产生刷新结果后，编辑反馈卡片会把“查看刷新结果”作为主操作前置。
- “回到动作编辑”保留为辅助操作；按钮继续复用现有主流程 action，不改变运行时输出和三值计算。
- 这让“查看结果 -> 定位动作 -> 修改动作 -> 查看刷新结果”的闭环入口更直观。

当前验证事实：

- Workbench 页面测试覆盖从编辑反馈卡片查看刷新结果，确认按钮指向刷新后的 runtime state point，并继续触发既有 `select-runtime-result` 主流程 action。
...
- 继续 UI 主流程能力块：优先检查资源曲线、日志和右侧三值详情在结果查看状态下的空间分配与主次层级，尽量把结果检查台整理成更接近 Endaxis 的紧凑工作流。

### 2026-07-09：UI 主流程资源曲线 - Selected Point Summary

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 资源曲线选中点卡片新增主数值摘要，把当前轨道、Delta、累计值和状态值提升为第一视线。
- 下方明细区收敛为定位与动作信息，避免曲线选点卡重复铺开 Delta / 累计 / 状态字段。
- 选中点的上一条/下一条导航、定位动作按钮和右侧三值详情同步继续复用现有主流程 action，不改变运行时输出。

当前验证事实：

- Workbench 页面测试覆盖从资源曲线选中运行点后，选中点卡片显示主摘要，并确认明细区只保留定位与动作。
...
- 继续 UI 主流程能力块：优先检查结果查看状态下日志面板与资源曲线面板的同步位置和滚动/空间占用，让曲线选点、日志选行和三值详情更像一个统一检查台。

### 2026-07-09：UI 主流程结果检查台 - Result Review Layout

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- Workbench 主流程区新增结果检查布局：没有运行结果选中时保持 overview；选中运行结果或存在待回看刷新结果时切换到 result-check。
- result-check 布局把更多横向空间分配给资源曲线，日志面板作为辅助列，减少结果查看时曲线选点被日志列表挤压的问题。
- 资源曲线、日志和右侧三值详情继续复用既有同步 action，本阶段只调整用户可见的空间主次，不改变运行时输出或数据结构。

当前验证事实：

- Workbench 页面测试覆盖初始 overview 布局，以及点击资源曲线点后切换到 result-check，确认资源区为主列、日志区为辅助列。
...
- 继续 UI 主流程能力块：优先把结果检查台里的曲线选点、日志选行、右侧三值详情和动作编辑入口做一次端到端操作体验复核，收敛仍然割裂的入口或空间问题。

### 2026-07-09：UI 主流程日志辅助列 - Selected Detail First

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 在结果检查布局中，日志面板作为辅助列时会把当前同步的日志详情前置到过滤器和日志列表上方。
- 普通 overview 日志浏览仍保持原有顺序；只有曲线点、日志行或刷新结果已经同步到右侧三值详情时，日志辅助列才优先呈现当前结果。
- 辅助列中的日志列表增加高度约束，避免长日志列表挤压当前结果详情和动作定位入口。

当前验证事实：

- Workbench 页面测试覆盖初始日志详情仍为 inline；从资源曲线选中运行点后，日志详情切换为 selected-first，并继续同步同一个 state point。
...
- 继续 UI 主流程能力块：优先复核右侧三值详情与主流程顶部操作、编辑反馈卡片之间的主次关系，确保查看结果、定位动作、修改动作、回看刷新结果形成一条更少跳转的闭环。

### 2026-07-09：UI 主流程顶部操作 - Contextual Primary Action

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- Workbench 结果检查区顶部主操作会根据当前闭环阶段显示更明确的动作文案。
- 选中运行结果时，顶部主操作从泛化“定位动作”调整为“编辑结果动作”；编辑产生刷新结果后，从泛化“回到结果点”调整为“查看刷新结果”。
- 行为仍复用既有 `focus-runtime-action` / `return-runtime-result` action，不改变运行时输出、三值结果或数据结构。

当前验证事实：

- Workbench 页面测试覆盖顶部主操作在结果选中状态显示“编辑结果动作”，在编辑后待回看状态显示“查看刷新结果”，并确认仍触发原有主流程 action。
...
### 2026-07-09：UI 主流程结果入口 - Cross Panel Action Alignment

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 资源曲线选中卡、日志详情和右侧三值详情的动作入口统一为“编辑结果动作”。
- 日志详情和右侧三值详情的刷新结果回看入口统一为“查看刷新结果”。
- 完整闭环测试覆盖曲线点、日志行、三值详情、动作编辑和刷新结果回看之间的入口文案与 action 同步；本阶段不改变运行时输出、三值结果或数据结构。

当前验证事实：

- Workbench 页面测试覆盖资源曲线、日志详情、右侧三值详情三处入口文案一致，并继续完成“曲线点 -> 日志行 -> 三值详情 -> 编辑动作 -> 查看刷新结果”的闭环。
...
### 2026-07-09：UI 主流程编辑定位 - Scroll To Edit Control

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 真实页面烟测发现：从三值详情点击“编辑结果动作”后，结果详情已同步但动作编辑控件仍在视口下方，用户需要自行滚动寻找“开始帧/开始时间”。
- 修复后，主流程成功进入动作编辑时会自动把对应编辑控件滚入视野；`startMs` / `durationMs` 优先定位到 60fps 帧控件。
- 行为仍复用现有 `focus-runtime-action` / `focus-edit-source` 主流程 action，不改变运行时输出、三值结果、公式或项目数据结构。

当前验证事实：

- Workbench DOM 测试挂载到 `document.body` 后覆盖“查看运行结果 -> 编辑结果动作”，确认页面会滚动到 `workbench-action-frame-control[data-edit-field="startMs"]`。
- 本阶段浏览器烟测用于定位问题；修复后的浏览器二次复核因浏览器连接在导航/标签读取时超时，未作为最终验收依据。
...
- 继续 UI 主流程能力块：优先处理结果检查区的横向溢出与整体空间压缩问题，让主流程在 1280px 桌面宽度下保持无横向滚动的紧凑可用状态。

### 2026-07-09：UI 主流程视口闭环 - 1280 Width Fit

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- Workbench 在 1280px 桌面宽度下不再因为根容器 `100vw` 计入滚动条宽度而出现页面级横向滚动。
- “查看运行结果 -> 编辑结果动作 -> 修改起始帧 -> 查看刷新结果”这条主流程在同一 1280px 视口下保持可走通，最终能定位到刷新后的结果点。
- 本阶段只调整顶层页面宽度适配，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- 浏览器 1280x720 实测修复前 `documentElement.scrollWidth=1280`、`clientWidth=1265`；修复后 `scrollWidth=1265`、`clientWidth=1265`，页面级横向溢出消失。
- 同一视口下走通“查看运行结果 -> 编辑结果动作 -> 起始帧改为 6f -> 查看刷新结果”，最终定位到 `enemyHpDamage|applied|action-0001|6|0`，全程 `hasHorizontalOverflow=false`。

验收结果：

- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，1 个测试文件、61 条测试。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk 体积提示。
- 浏览器 1280x720 主流程宽度验证：通过。
...
### 2026-07-09：UI 主流程结果密度 - Runtime Review Compact

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- `查看运行结果` 阶段的资源曲线面板更紧凑：标题、摘要、运行投影、曲线图和选中点详情的垂直间距减少。
- `查看运行结果` 阶段的日志面板更适合结果检查：原始事件列表高度收敛，模拟日志筛选、日志行、选中日志详情和操作按钮更短，选中点详情更早进入可见区域。
- 结果检查双列从固定较宽最小值改为可在 1280px 主列内收缩；移动端回退为单列。本阶段不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- 代码层已将结果检查两列从 `360px + 260px` 调整为 `300px + 220px` 的可收缩布局，避免结果区在 1280px 主列中过宽。
- `ResourceMonitorPanel` 与 `EventLogPanel` 只在 `data-flow-phase='runtime-result'` 下启用紧凑样式；动作编辑阶段仍保留原编辑优先布局。

验收结果：

- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，1 个测试文件、61 条测试。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk 体积提示。
- 构建产物 CSS 检查：通过，`dist/assets/Workbench-*.css` 包含结果检查双列 `300px/220px` 收缩规则、移动端单列回退，以及 `runtime-result` 阶段资源/日志紧凑样式。
- `git diff --check`：通过，仅有 LF/CRLF 提示。

下一步：

- 继续 UI 主流程能力块：围绕“曲线点 -> 日志行 -> 右侧三值详情”的联动做一次完整结果检查路径验收，优先修复仍需要明显滚动或定位不稳定的地方。

### 2026-07-09：UI 主流程结果定位 - Runtime Log Row Visibility

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- `查看运行结果` 阶段选中运行三值点后，事件日志面板会自动把对应的模拟日志行滚入日志列表可见范围。
- 这条定位适用于从曲线点、贡献拆分、运行结果入口等主流程来源切换 state point 后的日志同步，减少“右侧详情已变了但日志行还要手动找”的情况。
- 本阶段只加固结果检查路径里的可见定位，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- `EventLogPanel` 只在 `flowModel.phase === 'runtime-result'` 且存在选中 state point 时触发日志行滚动；动作编辑阶段不会触发。
- Workbench 闭环测试已覆盖“查看运行结果 -> 选中三值点 -> 对应模拟日志行 scrollIntoView -> 编辑结果动作”的主路径。
...
- 继续 UI 主流程能力块：做一次结果检查路径的浏览器复核；如果浏览器仍不稳定，则补充 DOM 层对“曲线点 -> 日志详情 -> 右侧三值详情”数据与定位状态的集中验收。

### 2026-07-09：UI 主流程结果验收 - Curve Point Review Path

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 固定 `查看运行结果` 中最常用的检查路径：点击资源曲线点后，主流程同步选中对应三值点、日志行、日志详情和右侧三值详情。
- 回归验收现在会确认对应模拟日志行被滚入日志列表可见范围，避免后续布局调整让用户点了曲线却还要手动找日志。
- 本阶段只补齐结果检查闭环验收，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- Workbench 闭环测试覆盖“资源曲线点 -> 选中 state point -> 对应模拟日志行 scrollIntoView -> 日志详情来源为 runtime-selected-detail -> 右侧三值详情同步”的路径。
- 这条验收与上一阶段的“打开结果入口自动定位日志”合并后，结果查看入口和曲线检查入口都有定位回归保护。
...
- 继续 UI 主流程能力块：优先做一次真实页面复核；如果浏览器仍不稳定，则把“日志行 -> 三值详情 -> 编辑动作 -> 返回刷新结果”也收成同类端到端验收。

### 2026-07-09：UI 主流程返回定位 - Log Result Return Path

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 固定 `日志行 -> 三值详情 -> 编辑动作 -> 返回刷新结果` 的结果检查闭环。
- 从模拟日志详情进入动作编辑并修改时间后，点击 `查看刷新结果` 会回到新的三值点，资源曲线、日志导航、动作贡献和右侧三值详情保持同步。
- 回归验收现在会确认刷新后的模拟日志行被滚入日志列表可见范围，避免用户返回结果后仍要手动寻找对应日志。
- 本阶段只补齐主流程可见定位验收，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- Workbench 闭环测试覆盖“日志行 -> 编辑结果动作 -> 修改动作时间 -> 查看刷新结果 -> 刷新后日志行 scrollIntoView -> 曲线/日志/贡献/详情同步”的路径。
- 这条验收与曲线点检查路径、打开结果入口定位路径合并后，结果查看、曲线检查、日志检查三个入口都有定位回归保护。
...
- 继续 UI 主流程能力块：优先做真实页面复核；若浏览器仍不稳定，再补一条围绕“排轴动作编辑 -> 运行模拟 -> 查看结果 -> 编辑动作 -> 返回刷新结果”的更高层 Workbench 集中验收。

### 2026-07-09：UI 主流程结果复核布局 - Refreshed Result Review

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- `查看运行结果` 与 `查看刷新结果` 现在共享同一套结果检查布局规则。
- 从动作编辑返回刷新结果后，资源曲线与模拟日志仍保持结果检查区的前置、紧凑双列布局，避免刷新后回到较松散的编辑态排布。
- 本阶段只统一 UI 主流程复核态布局，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- Workbench 可见闭环测试覆盖初次查看结果与返回刷新结果后都处于 `result-check` 布局，且资源区为主列、日志区为次列。
- 构建产物 CSS 已确认 `runtime-result` 与 `edit-result-review` 都命中主流程结果检查、资源曲线和模拟日志的紧凑布局选择器。

验收结果：

- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，1 个测试文件、61 条测试。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk 体积提示。
- Workbench CSS `edit-result-review` 选择器检查：通过。
- `git diff --check`：通过，仅有 LF/CRLF 提示。

下一步：

- 继续 UI 主流程能力块：优先做真实页面复核；如果浏览器仍不稳定，则补一条页面级布局/溢出回归检查，固定“动作编辑 -> 查看结果 -> 返回刷新结果”在桌面宽度下不会横向溢出且结果区保持可见。

### 2026-07-09：UI 主流程真实页面复核 - Desktop Result Loop

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用本地真实页面复核 `排轴动作编辑 -> 查看运行结果 -> 编辑结果动作 -> 查看刷新结果` 的桌面主流程。
- 在 1280×720 视口下，初次查看运行结果和返回刷新结果都保持结果检查区前置，资源曲线为主列、模拟日志为次列，页面没有横向溢出。
- 返回刷新结果后，刷新后的模拟日志行处于可见范围，右侧三值详情选中刷新后的 state point。
- 本阶段只记录真实页面验收结果，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- 本地开发页面：`http://127.0.0.1:5177/#/workbench`。
- 初始动作编辑态：`phase=action-edit`，横向溢出 `0px`。
- 查看运行结果后：`phase=runtime-result`，`layout=result-check`，资源区 `primary`，日志区 `secondary`，横向溢出 `0px`，结果区顶端约 `35px`，时间轴下移到结果区之后。
- 修改动作起始帧为 `6` 后：`phase=edit-result-ready`，出现 `查看刷新结果`，横向溢出 `0px`。
- 查看刷新结果后：`phase=edit-result-review`，`layout=result-check`，资源区 `primary`，日志区 `secondary`，`resultContextStatus=refreshed-edit-result`，横向溢出 `0px`，刷新后的日志行 `data-selected=true` 且位于视口内。
...
### 2026-07-09：UI 主流程窄屏闭环 - Narrow Result Loop

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 窄屏下结果检查区保持单列主流程，用户仍能沿着 `查看结果 -> 曲线点 -> 日志/三值详情 -> 回改动作 -> 查看刷新结果` 走完整闭环。
- 资源曲线的选中信息、三值卡片、导航和图例在窄屏下纵向展开，避免结果点信息被横向挤压。
- 模拟日志的筛选、详情、回编辑和回结果按钮在窄屏下改为可点击的整行控件，避免用户在小窗口里找不到下一步操作。
- 本阶段只补齐 UI 主流程窄屏可操作性，不改变运行时输出、三值结果、公式、证据字段或数据结构。

当前验证事实：

- `supports the visible workbench loop across curve, log, detail, edit, and refreshed result` 继续覆盖完整可操作路径。
- 新增 `guards the narrow result loop layout contract` 固定窄屏结果闭环依赖的 CSS 布局合同。

验收结果：

- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，1 个测试文件、63 条测试。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk 体积提示。
- `git diff --check`：通过，仅有 LF/CRLF 提示。
...
### 2026-07-09：UI 主流程选中动作编辑回看 - Selected Action Edit Return

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户从时间轴或动作列表定位到某个运行结果后，可以直接在属性面板修改当前动作，并通过“查看刷新结果”回到修改后的结果点。
- 直接编辑前会保留原运行结果点作为来源；修改导致旧结果点被重算替换后，刷新结果仍能回到新结果点。
- 回看刷新结果后，资源曲线、日志导航、三值详情、HP 贡献拆分、动作列表选中项和时间轴选中项会同步到同一个动作结果。
- 本阶段只补齐 UI 主流程闭环，不改变三值输出、公式、证据字段或持久数据结构。

当前验证事实：

- 修复 `Workbench.vue`：当前动作修改前捕获已选中的运行结果点，并交给现有编辑来源/刷新结果链路。
- 扩展 `e2e/workbench-continuous-edit.spec.js`：覆盖时间轴选中 `action-0002` -> 修改开始帧 -> 查看刷新结果 -> 曲线/日志/详情/贡献拆分同步。

验收结果：

- `npm run test:e2e`：通过，1 条浏览器级烟测。
- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，1 个测试文件、67 条测试。
- `npx prettier --check src/views/Workbench.vue e2e/workbench-continuous-edit.spec.js PROJECT_MANUAL.md`：通过。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk 体积提示。
- `git diff --check`：通过，仅有 LF/CRLF 提示。
...
### 2026-07-09：UI 主流程窄屏闭环 - Narrow Viewport Loop

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 390px 窄屏视口下，用户可以打开运行结果、从贡献拆分进入动作编辑、修改动作帧，并回到刷新后的结果点。
- 窄屏回看刷新结果后，资源曲线、模拟日志、三值详情、动作贡献拆分、动作列表和时间轴仍同步到同一个动作结果。
- 浏览器级回归会持续检查主流程关键阶段没有页面横向溢出。
- 本阶段只固定窄屏主流程可用性，不改变三值输出、公式、证据字段或持久数据结构。

当前验证事实：

- 扩展 `e2e/workbench-continuous-edit.spec.js`：新增 390px 宽度下的贡献拆分 -> 编辑动作 -> 查看刷新结果闭环。
- 窄屏测试覆盖入口可点击、编辑来源同步、刷新结果定位、曲线/日志/详情/贡献拆分同步和 `pageOverflowX === 0`。
...
### 2026-07-09：UI 主流程时间轴编辑入口 - Timeline Edit Entry

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户在时间轴中选中某个运行结果动作后，可以直接从该动作块上的编辑图标进入动作编辑。
- 编辑入口复用现有 runtime action edit command；不新建流程，不改变三值输出。
- 修改动作后仍能通过“查看刷新结果”回到刷新后的结果点，并同步资源曲线、模拟日志、三值详情、贡献拆分、动作列表和时间轴。
- 本阶段只补齐时间轴上的高频回编入口，不改变公式、证据字段或持久数据结构。

当前验证事实：

- `TimelineGridPreview.vue` 在当前运行结果动作块上显示本地编辑图标，并将点击派发到现有主流程 action。
- `e2e/workbench-continuous-edit.spec.js` 现在覆盖：点击时间轴动作 -> 点击动作块编辑入口 -> 修改动作帧 -> 查看刷新结果。
...
### 2026-07-09：UI 主流程删除回落闭环 - Delete Result Fallback Loop

本阶段属于：UI 主流程。

完成的可用能力：

- 用户在多动作运行结果中查看某个动作的曲线、日志和三值详情后，可以直接删除这个动作。
- 被查看动作删除后，Workbench 会自动定位到剩余的可用运行结果，资源曲线、模拟日志、三值详情、贡献拆分和动作选择保持同步。
- 用户可以从回落后的结果详情继续进入动作编辑，并通过“查看刷新结果”回到刷新后的运行点。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、保存数据或导入导出结构。

当前验证事实：

- 新增浏览器级闭环场景：`多动作运行结果 -> 删除当前结果动作 -> 自动落到剩余结果 -> 编辑回看刷新结果`。
- 该场景复用现有 runtime sync request 和主流程 action，不新增内部抽象层。

验收结果：

- `npm run test:e2e -- --grep "deleting the focused action"`：通过，1 条浏览器级闭环测试。
- `npm run test -- --run`：通过，38 个测试文件、269 条测试。
- `npm run test:e2e`：通过，6 条浏览器级烟测。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk size 提示。
...
### 2026-07-09：UI 主流程时间轴状态点闭环 - Timeline State Marker Loop

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户在时间轴上把曲线显示范围切回“全部”后，可以直接点击状态曲线 marker 定位到对应运行结果。
- 时间轴 marker 选择后，三值详情、资源曲线、模拟日志、贡献拆分、动作列表和时间轴选中态保持同步。
- 用户可以从该结果进入动作编辑，并通过“查看刷新结果”回到刷新后的运行点。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、保存数据或导入导出结构。

当前验证事实：

- 新增浏览器级闭环场景：`多动作运行结果 -> 时间轴曲线视图切回全部 -> 点击状态曲线 marker -> 曲线/日志/贡献同步 -> 编辑动作 -> 查看刷新结果`。
- 该场景复用既有时间轴状态曲线 marker、主流程 action 和 runtime sync request，不新增内部抽象层。

验收结果：

- `npm run test:e2e -- --grep "timeline state curve marker"`：通过，1 条浏览器级闭环测试。
- `npm run test -- --run`：通过，38 个测试文件、271 条测试。
- `npm run test:e2e`：通过，9 条浏览器级烟测。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk size 提示。
- `npx prettier --check PROJECT_MANUAL.md e2e/workbench-continuous-edit.spec.js`：通过。
- `git diff --check`：通过，仅有 LF/CRLF 提示。
...
- 继续 UI 主流程能力块：补齐 Analysis 状态曲线列表/帧组入口到编辑回跳的浏览器级闭环；若 UI 主流程入口都稳定，再转入生成层或运行时层边界收敛。

### 2026-07-09：UI 主流程 Analysis 状态曲线列表闭环 - Analysis State Curve Point Loop

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户在 Analysis 面板把状态曲线视图切回“全部”后，可以直接点击状态曲线列表中的已用状态点定位到对应运行结果。
- Analysis 列表点选择后，三值详情、资源曲线、模拟日志、贡献拆分、动作列表和时间轴选中态保持同步。
- 用户可以从该结果进入动作编辑，并通过“查看刷新结果”回到刷新后的运行点。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、保存数据或导入导出结构。

当前验证事实：

- 新增浏览器级闭环场景：`多动作运行结果 -> Analysis 状态曲线视图切回全部 -> 点击已用状态点 -> 曲线/日志/贡献同步 -> 编辑动作 -> 查看刷新结果`。
- 该场景复用既有 Analysis 状态曲线列表、主流程 action 和 runtime sync request，不新增内部抽象层。

验收结果：

- `npm run test:e2e -- --grep "analysis state curve point"`：通过，1 条浏览器级闭环测试。
- `npm run test -- --run`：通过，38 个测试文件、271 条测试。
- `npm run test:e2e`：通过，10 条浏览器级烟测。
- `npm run build`：通过，仅有既有 Sass `@import` 弃用提示和 chunk size 提示。
- `npx prettier --check PROJECT_MANUAL.md e2e/workbench-continuous-edit.spec.js`：通过。
- `git diff --check`：通过，仅有 LF/CRLF 提示。
...
### 2026-07-09：UI 主流程可见闭环 - 筛选后找回运行结果

本阶段属于：UI 主流程。

完成的可用能力：

- 用户现在可以在资源曲线/模拟日志/三值详情定位到某个运行结果后切换日志筛选，即使当前筛选暂时隐藏该结果，也能通过现有入口找回对应日志。
- 找回后同一个状态点会重新同步到资源曲线、模拟日志、三值详情，并且可以继续回到动作编辑，修改后再查看刷新后的结果定位。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、保存数据、导入导出结构或 UI 文案。

当前验证事实：

- `workbench-continuous-edit.spec.js` 在完整浏览器路径中固定：运行模拟 -> 点击曲线点 -> 点击日志行 -> 切换到排除当前结果的日志筛选 -> 找回当前结果 -> 查看三值贡献 -> 回动作编辑 -> 查看刷新后的曲线/日志结果。
- 找回当前结果后，主流程 runtime output consistency 仍为 consistent，曲线与日志选择仍指向同一个状态点。
...
- 建议继续 UI 主流程：补一条更接近成品使用的“多动作排轴编辑 -> 运行模拟 -> 曲线/日志/详情横向导航 -> 修改中间动作 -> 结果重新定位”端到端路径；若 UI 主流程已经够稳，则转入生成层把 `Action -> Hit -> ThreeValueDelta` 的统一生成入口做成稳定合同。

### 2026-07-09：UI 主流程可见闭环 - 多动作曲线横向浏览与中段编辑

本阶段属于：UI 主流程。

完成的可用能力：

- 用户现在可以在三段动作排轴运行后，从资源曲线详情的上一项/下一项横向浏览不同动作的运行结果。
- 横向浏览到中间动作后，可以直接从资源曲线详情进入动作编辑；修改起始帧后查看刷新结果，曲线、日志、三值详情和动作结果行会重新定位到刷新后的中间动作状态点。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、保存数据、导入导出结构或 UI 文案。

当前验证事实：

- `workbench-continuous-edit.spec.js` 新增浏览器路径：多动作排轴 -> 运行模拟 -> 资源曲线详情上一项回到 `action-0002` -> 下一项回到 `action-0003` -> 再回到 `action-0002` -> 从曲线详情进入编辑 -> 查看刷新结果。
- 刷新后确认主流程 runtime output consistency 仍为 consistent，资源曲线与模拟日志选择同一个状态点，动作结果行也指向刷新后的状态点。

验收结果：

- `npx playwright test e2e/workbench-continuous-edit.spec.js -g "keeps multi-action resource chart navigation tied to middle edit return"`：通过，1 条浏览器级主流程测试。
...
### 2026-07-09：UI 主流程可见闭环 - 重置后重新编辑

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 浏览器级主流程现在覆盖保存草稿、刷新恢复、重置草稿、重新运行模拟、再次编辑动作并查看刷新结果的完整路径。
- 用户重置草稿后会回到默认单动作排轴，localStorage 草稿被清空；随后仍可运行模拟、从结果详情进入动作编辑、修改起始帧并回到刷新结果。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、生成层合同、运行时输出、保存 schema 或 DATA_STRUCTURE_CHANGES。

当前验证事实：

- `workbench-continuous-edit.spec.js` 新增重置后再编辑路径：保存 `action-0002` 多动作草稿 -> 页面刷新恢复 -> 重置 -> 运行默认 `action-0001` -> 编辑起始帧 -> 查看刷新结果。
- 重置后确认 `scenario-action-count = 1 action`、选中 `action-0001`、等级和动作形态回到默认值，localStorage 草稿为空。
- 再次编辑后确认资源曲线、模拟日志、三值详情和动作结果定位到刷新后的 `action-0001` 状态点，草稿状态为“有未保存改动”。

验收结果：

- `npx playwright test e2e/workbench-continuous-edit.spec.js -g "keeps reset draft usable for a fresh runtime edit loop"`：通过，1 条浏览器级主流程闭环测试。
- `npx prettier --check PROJECT_MANUAL.md e2e/workbench-continuous-edit.spec.js`：通过。
- `git diff --check`：通过，仅有 LF/CRLF 提示。
...
### 2026-07-09：UI 主流程可见闭环 - 批次复制与结果定位

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 批次管理新增“复制批次”，用户可以把生成出的整组动作复制成新的批次，保留组内相对间隔并整体放到原批次之后。
- 如果用户已经在运行结果页，复制批次后会直接刷新运行结果并定位到新批次第一条动作；曲线、日志、三值详情和贡献拆分会同步到新的运行状态点。
- 用户现在能完成：生成/载入批次 -> 运行模拟 -> 复制批次 -> 查看复制批次结果 -> 编辑该动作 -> 回到刷新后的结果定位。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、生成层合同、运行时输出或保存 schema。

当前验证事实：

- 复制批次会生成新的 `segment-batch`，并标记为 `batch-copy`，原批次不被修改。
- 复制后 Workbench 会选中新批次第一条动作；在 runtime review 中会同步到对应曲线点、日志行和三值详情。
...
### 2026-07-09：UI 主流程可见闭环 - 批次删除撤销/重做

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 批次结果入口后的删除操作现在纳入浏览器级主流程验收：误删批次后可以撤销恢复批次、恢复原运行结果定位，再重做删除并回到可用的回退结果。
- 同一条可见路径覆盖：批次查看结果 -> 删除批次 -> 查看回退结果 -> 撤销恢复批次结果 -> 重做删除 -> 编辑回退动作 -> 回到刷新后的结果定位。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、生成层合同、运行时输出或保存 schema。

当前验证事实：

- 撤销删除后，批次卡片和批次动作恢复，运行详情、曲线、日志和贡献拆分重新指向原批次状态点。
- 重做删除后，批次再次移除，运行结果稳定回退到剩余可用动作，随后仍可进入动作编辑并返回刷新后的结果定位。
...
- 继续 UI 主流程能力块：补齐批次复制后的撤销/重做与结果定位验收，随后再把这一组批次主流程沉淀成更稳定的 Workbench 回归套件。

### 2026-07-09：UI 主流程可见闭环 - 批次复制撤销/重做

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 批次复制后的撤销/重做现在纳入浏览器级主流程验收：复制批次后可以撤销回原批次结果，再重做回复制批次结果。
- 同一条可见路径覆盖：批次查看结果 -> 复制批次 -> 查看复制批次结果 -> 撤销回原批次结果 -> 重做回复制批次结果 -> 编辑复制动作 -> 回到刷新后的结果定位。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、生成层合同、运行时输出或保存 schema。

当前验证事实：

- 撤销复制后，新批次动作移除，运行详情、曲线、日志和贡献拆分回到原批次状态点。
- 重做复制后，新批次动作恢复，运行结果重新定位到复制批次第一条动作；随后仍可进入动作编辑并返回刷新后的结果定位。
...
### 2026-07-09：UI 主流程可见闭环 - 运行复盘快速切换

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 用户现在可以从排轴动作编辑进入运行模拟，在运行复盘中用前后方向快速切换相邻动作结果，并立即看到资源曲线、日志、三值详情和当前动作定位同步更新。
- 这条路径复用既有主流程运行结果选择逻辑，不改变三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值或草稿保存 schema。
- 本阶段没有新增数据结构；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test -- --run src/__tests__/views/Workbench.test.js`：通过，71 条测试。
- `npm run test:e2e:workbench-flow`：通过，5 条浏览器级主流程测试。

下一步：

- 继续 UI 主流程能力块：补完整编辑体验中仍缺的用户可见路径，例如动作插入/重排/批次操作后的复盘定位统一验收；如果主流程稳定，则转入生成层整理 `Action -> Hit -> ThreeValueDelta` 的统一生成入口。

### 2026-07-09：UI 主流程可见闭环 - 时间轴微调回跳验收

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 主流程回归现在覆盖“动作列表定位中段动作 -> 从时间轴进入编辑 -> 用 1 帧步进微调 -> 查看刷新结果 -> 曲线/日志/三值详情同步到新结果”的完整路径。
- 这条验收固定了排轴时最常见的微调闭环：用户不需要重新从头找结果，回到刷新结果后仍能继续在资源曲线和日志里复盘。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值或草稿保存 schema；没有新增数据结构，`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow`：通过，6 条浏览器级主流程测试。

下一步：

- 继续 UI 主流程能力块：补动作插入/重排/批次操作后的同类复盘定位闭环；如果 UI 主流程已稳定，则转入生成层整理 `Action -> Hit -> ThreeValueDelta` 的统一生成入口。

### 2026-07-09：UI 主流程可见闭环 - 删除当前动作后的复盘回退

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 主流程回归现在覆盖“多动作运行复盘 -> 删除当前结果对应动作 -> 自动回退到仍可用动作结果 -> 曲线/日志/三值详情同步 -> 继续编辑并查看刷新结果”的路径。
- 这条验收固定了排轴编辑中的安全回退体验：用户删掉当前动作后不会停在失效结果上，后续仍能沿着同一条复盘/编辑闭环继续操作。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值或草稿保存 schema；没有新增数据结构，`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow`：通过，7 条浏览器级主流程测试。

下一步：

- 继续 UI 主流程能力块：补动作插入/批次位移后的同类复盘定位闭环；如果 UI 主流程已稳定，则转入生成层整理 `Action -> Hit -> ThreeValueDelta` 的统一生成入口。

### 2026-07-09：UI 主流程可见闭环 - 批次位移后的复盘定位

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 主流程回归现在覆盖“查看生成批次结果 -> 整体 +30 帧位移 -> 当前结果刷新为新 state point -> 曲线/日志/三值详情同步 -> 继续编辑并查看刷新结果”的路径。
- 这条验收固定了批次排轴调整后的复盘体验：用户移动一组动作后，当前复盘对象会跟着刷新，不会停在旧时间点或旧运行结果。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值或草稿保存 schema；没有新增数据结构，`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow`：通过，8 条浏览器级主流程测试。

下一步：

- 继续 UI 主流程能力块：补动作插入后的同类复盘定位闭环；如果 UI 主流程已稳定，则转入生成层整理 `Action -> Hit -> ThreeValueDelta` 的统一生成入口。

### 2026-07-09：UI 主流程可见闭环 - 动作插入后的复盘定位

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 主流程回归现在覆盖“运行复盘 -> 新增动作 -> 新动作结果立即进入复盘 -> 曲线/日志/三值详情同步 -> 编辑新动作并查看刷新结果”的路径。
- 这条验收固定了排轴扩展时的插入闭环：用户在复盘过程中新增动作后，可以直接检查新增动作的三值变化，再继续修改并回到刷新后的结果。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值或草稿保存 schema；没有新增数据结构，`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow`：通过，9 条浏览器级主流程测试。

下一步：

- UI 主流程可见闭环已覆盖运行、曲线/日志/详情、微调、删除回退、批次复制/位移/删除和动作插入；建议转入生成层整理 `Action -> Hit -> ThreeValueDelta` 的统一生成入口，保留 `workbench-flow` 作为主流程回归守门。

### 2026-07-09：生成层 - Generation Entry 标准输出

本阶段属于：生成层。

完成的可用能力：

- `Action -> Hit -> ThreeValueDelta` 现在作为 `generationOutputs.outputs.generationEntry` 的标准入口输出，聚合 `generationInput`、`standardContract`、`actions`、`hits`、`deltas` 和 `runtimeInputSource`。
- runtime input 现在优先从 `generationEntry` 读取 `runtimeInputSource / standardContract / deltas`，旧直出字段继续作为兼容后备。
...
- 结构变化已记录到 `DATA_STRUCTURE_CHANGES.md` 的 `328. 生成层标准入口输出：Generation Entry Standard Output`。
...
### 2026-07-09：UI 主流程可见闭环 - 草稿恢复后的复盘续接

本阶段属于：UI 主流程。

完成的可用能力：

- 主流程回归现在覆盖“多动作排轴 -> 选择运行结果 -> 编辑动作 -> 保存草稿 -> 刷新恢复 -> 打开恢复后的运行结果 -> 再次编辑 -> 查看刷新后的曲线/日志/详情同步”的完整用户路径。
- 这条验收把保存/恢复从单纯数据持久化提升为可继续操作的复盘闭环：用户恢复草稿后不会丢失主流程上下文，可以继续定位结果、修改动作并查看刷新后的结果。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "saved draft restore"`：通过，覆盖草稿恢复后的复盘续接闭环。

下一步：

- 继续 UI 主流程能力块：检查重置草稿、撤销/重做后的持久化恢复、以及跨入口复盘导航是否还缺主流程级守门；优先补用户可见闭环，不继续新增内部抽象。

### 2026-07-09：UI 主流程可见闭环 - 跨入口复盘导航

本阶段属于：UI 主流程。

完成的可用能力：

- 主流程回归现在覆盖“结果列表入口 -> 贡献面板导航 -> 曲线选择导航 -> 详情面板导航 -> 回到动作编辑 -> 查看刷新后的曲线/日志/详情同步”的跨入口复盘闭环。
- 这条验收固定了 Endaxis 式复盘体验的关键路径：用户不论从结果列表、贡献拆分、资源曲线还是详情面板进入复盘，都能定位同一个运行结果，并继续修改动作后回到刷新结果。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "result review entrances"`：通过，覆盖跨入口复盘导航闭环。

下一步：

- 继续 UI 主流程能力块：检查重置草稿和撤销/重做后的恢复流程是否也需要进入主流程守门；仍优先补用户可见闭环，不新增内部抽象。

### 2026-07-09：UI 主流程可见闭环 - 重置草稿后的新复盘闭环

本阶段属于：UI 主流程。

完成的可用能力：

- 主流程回归现在覆盖“保存草稿 -> 刷新恢复 -> 重置草稿 -> 重新运行默认动作 -> 编辑动作 -> 查看刷新后的曲线/日志/详情同步”的完整路径。
- 这条验收固定了重置草稿后的新工作流：用户清空恢复状态后，可以从默认动作重新进入运行复盘并继续编辑，不会停在旧草稿或旧运行结果上。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "reset draft usable"`：通过，覆盖重置草稿后的新复盘闭环。

下一步：

- 继续 UI 主流程能力块：检查撤销/重做与持久化恢复、窄视口编辑回流是否也需要进入主流程守门；仍优先补用户可见闭环，不新增内部抽象。

### 2026-07-09：UI 主流程可见闭环 - 窄视口复盘编辑回流

本阶段属于：UI 主流程。

完成的可用能力：

- 主流程回归现在覆盖 390px 窄视口下的“运行模拟 -> 查看曲线/日志同步 -> 从贡献详情编辑动作 -> 查看刷新后的曲线/日志/详情同步”路径。
- 这条验收固定了移动/窄屏工作台的复盘编辑体验：用户在窄视口下不会因为布局回流而丢失结果定位，也不会出现页面横向溢出。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "narrow viewport"`：通过，覆盖窄视口复盘编辑回流闭环。
...
### 2026-07-09：UI 主流程可见闭环 - 主流程面板复盘闭环

本阶段属于：UI 主流程。

主流程缺口检查：

- 现有界面已经具备“运行模拟”入口、曲线点选择、日志行选择、三值详情、编辑结果动作、查看刷新结果这些可见操作。
- 当前最需要守住的缺口是：没有一条专门只通过主流程面板按钮串起完整复盘路径的回归；后续改动可能让主路径入口或返回结果断开，但被其它分散入口的测试掩盖。

完成的可用能力：

- 主流程回归新增“主流程面板复盘闭环”：用户从主流程面板点击“运行模拟”，在资源曲线和事件日志中定位同一三值点，再从主流程面板点击“编辑结果动作”，修改动作帧后点击“查看刷新结果”，最终回到刷新后的曲线、日志、三值详情和结果列表定位。
- 这条验收固定了 Endaxis 式工作流中最核心的一条可见主路径：排轴动作编辑 -> 运行模拟 -> 查看资源曲线 -> 查看日志/三值详情 -> 回到动作修改 -> 看到刷新后的结果定位。
- 本阶段不改变 UI 可见文案、三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "Workbench flow panel edit-result loop"`：通过，覆盖主流程面板复盘闭环。
- `npm run test:e2e:workbench-flow`：通过，14 条 `@workbench-main-flow` 主流程回归全部通过。
...
### 2026-07-10：UI 主流程可见闭环 - 刷新结果连续回改

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- 现有界面已经能完成“排轴动作编辑 -> 运行模拟 -> 查看资源曲线 -> 查看日志/三值详情 -> 回到动作修改 -> 看到刷新后的结果定位”。
- 当前最需要守住的可用体验缺口是：用户查看第一次刷新结果后，还要能继续回到动作编辑，再次修改并看到第二次刷新后的曲线、日志、三值详情和结果列表定位一起更新。

完成的可用能力：

- 主流程回归新增“刷新结果连续回改”路径：运行模拟 -> 查看曲线/日志定位 -> 编辑结果动作 -> 查看刷新结果 -> 从刷新结果继续修改动作 -> 再次查看刷新结果。
- 这条验收固定了 Workbench 的可循环复盘体验：用户不需要退出主流程，也不需要从其它零散入口重新找结果点，就能在同一条 Endaxis 式路径里连续改动作、看结果、再回改。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "refreshed runtime results editable"`：通过，覆盖刷新结果连续回改闭环。
- `npm run test:e2e:workbench-flow`：通过，16 条 `@workbench-main-flow` 主流程回归全部通过。

下一步：

- 继续 UI 主流程能力块：把真实页面里的主流程可用性集中到“紧凑编辑、结果定位、贡献拆分、曲线交互、日志详情”这一整条体验上；不要回到纯内部抽象或微型提示阶段。

### 2026-07-10：UI 主流程可见闭环 - 回改入口语义统一

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- 顶部主流程按钮已经能在刷新结果页显示“继续修改动作”，但资源曲线选中点、事件日志详情、三值详情里的回改按钮仍各自保持“编辑结果动作”。
- 这会让用户在刷新结果页继续回改时看到多个入口表达不一致，削弱“看结果 -> 继续改 -> 再看结果”的同一条主流程感。

完成的可用能力：

- 曲线、日志、三值详情三个回改入口现在统一消费主流程 review operation label；刷新结果页显示“继续修改动作”，普通运行结果仍显示“编辑结果动作”。
- 新增端到端断言固定刷新结果页里顶部主操作、曲线入口、日志入口、三值详情入口四处一致后，再继续修改并查看第二次刷新结果。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test -- --run src\__tests__\features\workbenchMainFlowActions.test.js src\__tests__\features\RuntimeSelectedDetailPanel.test.js src\__tests__\views\Workbench.test.js`：通过，3 个测试文件、103 条测试。
- `npm run test:e2e:workbench-flow`：通过，16 条 `@workbench-main-flow` 主流程回归全部通过。
...
### 2026-07-10：UI 主流程可见闭环 - 三值详情返回刷新结果

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- 主流程面板、资源曲线、事件日志、贡献拆分和连续回改路径已有端到端覆盖。
- 仍需要固定一条用户自然会走的支路：从事件日志进入三值详情后，直接从三值详情回到动作编辑，并从同一详情区域查看刷新后的结果定位。

完成的可用能力：

- 新增端到端回归固定“三值详情支路”：运行模拟 -> 点击日志查看三值详情 -> 从详情编辑动作 -> 在详情区域点击“查看刷新结果” -> 回到刷新后的曲线、日志、三值详情和结果列表定位。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、运行日志行、曲线数值、UI 文案、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- -g "runtime detail panel"`：通过，覆盖三值详情返回刷新结果支路。
- `npm run test:e2e:workbench-flow`：通过，17 条 `@workbench-main-flow` 主流程回归全部通过。

下一步：

- 继续 UI 主流程能力块：在不新增微型状态提示的前提下，优先检查真实页面的紧凑操作、结果定位可读性、曲线交互和日志详情是否已经形成稳定的 Endaxis 式主流程体验。

### 2026-07-10：UI 主流程可见闭环 - 日志内结果导航

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- 资源曲线选中点和三值详情已有上一条/下一条运行结果导航。
- 事件日志只有同步状态和当前序号，用户查看日志细节时不能直接在日志区域切换相邻结果；这会打断“查看日志/详情 -> 继续定位结果 -> 回到动作修改”的复盘节奏。

完成的可用能力：

- 事件日志导航条新增上一条/下一条结果按钮，直接复用主流程 runtime state point 选择动作。
- 用户现在可以从日志区域连续切换相邻运行结果，曲线、日志选中行、三值详情和主流程结果定位会同步跳转。
- 本阶段不改变三值计算结果、公式、倍率、证据字段、运行日志数据、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test -- --run src\__tests__\features\EventLogPanel.test.js`：通过，1 个测试文件、2 条测试。
- `npm run test:e2e:workbench-flow -- -g "runtime log navigation"`：通过，覆盖三动作场景下从日志切到上一条/下一条结果并同步曲线、日志和详情。
...
### 2026-07-10：UI 主流程可见闭环 - 资源曲线刷新结果回跳

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- 资源曲线选中点已有“编辑结果动作”，但从曲线卡片进入编辑后，刷新结果回跳主要依赖主流程条、日志或侧栏；曲线卡片自身不是完整闭环入口。

完成的可用能力：

- 资源曲线选中卡片新增刷新结果回跳入口，用户可以从曲线点直接进入动作编辑，并从同一曲线区域查看刷新后的结果定位。
- 曲线卡片在刚完成编辑时优先定位刷新后的 pending 结果；若用户手动点击其他曲线点，则尊重手动选择，不强制抢回 pending 点。
- 本阶段不改变三值计算结果、公式、倍率、applied delta 筛选、运行日志行、曲线数值、运行期数据结构或草稿保存 schema；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test -- --run src\__tests__\views\Workbench.test.js`：通过，1 个测试文件、71 条测试。
- `npm run test:e2e:workbench-flow -- -g "visible curve-log-detail"`：通过，覆盖资源曲线 -> 日志/详情 -> 编辑 -> 曲线卡片查看刷新结果的可见闭环。
- `npm run test:e2e:workbench-flow`：通过，18 条 `@workbench-main-flow` 主流程回归全部通过。
...
### 2026-07-10：UI 主流程可见闭环 - 多动作日志筛选找回

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 多动作复盘时，用户可以在资源曲线定位到中间动作结果，再把日志筛到其他动作；当前结果即使被筛选隐藏，也能从日志区域找回。
- 找回日志后，用户可以从日志三值详情进入动作编辑，并通过日志区域的刷新结果入口回到更新后的结果定位。
- 该闭环在窄屏视口下纳入主流程回归，确认曲线选点、日志选中行、三值详情、动作编辑和刷新结果定位不会产生横向溢出。
- 本阶段不改变三值计算结果、公式、倍率、运行期数据结构、草稿保存 schema 或 UI 文案；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- --grep "recovers filtered multi-action logs"`：通过。
...
### 2026-07-10：UI 主流程可见闭环 - 刷新结果继续回改入口

本阶段属于：UI 主流程可见闭环。

主流程缺口检查：

- Workbench 已具备运行模拟、资源曲线、日志/三值详情、动作编辑和刷新结果定位能力；但刷新结果复核阶段顶部主流程按钮仍固定显示“编辑结果动作”，没有使用模型里已有的“继续修改动作”主操作标签。

完成的可用能力：

- 用户从资源曲线或日志进入三值详情后，可以回到动作编辑；修改动作产生刷新结果后，顶部主流程入口会显示“继续修改动作”，并直接回到该刷新结果对应的动作编辑状态。
- 端到端 Workbench 用例已固定完整可见闭环：运行模拟 -> 资源曲线选点 -> 日志/三值详情 -> 编辑动作 -> 查看刷新结果 -> 继续修改动作。
- 本阶段不改变三值计算结果、公式、倍率、运行期数据结构、草稿保存 schema 或证据字段；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test -- src/__tests__/views/Workbench.test.js src/__tests__/features/WorkbenchFlowPanel.test.js`：通过，2 个测试文件、76 条测试。
...
### 2026-07-10：UI 主流程可见闭环 - 窄屏顶部回改守门

本阶段属于：UI 主流程可见闭环。

完成的可用能力：

- 窄屏 Playwright 主流程守门现在覆盖刷新结果后的顶部主流程条：用户在移动宽度下查看刷新结果后，可以从顶部“继续修改动作”入口进入下一轮动作编辑。
- 该路径继续验证第二次编辑会生成新的刷新结果，并保持资源曲线、日志、三值详情同步；每个关键节点都检查页面没有横向溢出。
- 本阶段不改变三值计算结果、公式、倍率、运行期数据结构、草稿保存 schema 或 UI 文案；`DATA_STRUCTURE_CHANGES.md` 无需追加。

验收结果：

- `npm run test:e2e:workbench-flow -- --grep "keeps the edit result loop usable at a narrow viewport"`：通过，1 条窄屏真实浏览器主流程回归。
- `npm run test:e2e:workbench-flow`：通过，21 条 `@workbench-main-flow` 主流程回归全部通过。
- `git diff --check`：通过，仅出现 Windows 换行提示。
