...
describe('verified combat mechanics runtime', () => {
  ...
  it('settles the breaking packet before Break and a later same-frame packet after Break', () => {
    ...
    expect(
      BigInt(hits[1].payload.formulaBreakdown.verifiedResult.preShieldRaw)
    ).toBe(
      BigInt(hits[0].payload.formulaBreakdown.verifiedResult.preShieldRaw) * 2n
    );
    expect(
      Math.abs(
        hits[1].payload.effectiveHpDamage -
  ...
  it('keeps the verified weakness multiplier and clamp order with independent loss percentages', () => {
    ...
    expect(hit.payload.formulaBreakdown.weaknessInput).toMatchObject({
      ...
      maximumWeakness: 26822.0077,
    });
    expect(hit.payload.hpLossPercent).toBe(
      Number((hit.payload.rawDamage / 86778.6984).toFixed(6))
    );
    expect(hit.payload.toughnessLossPercent).toBe(
      Number((hit.payload.toughnessDamage / 26822.0077).toFixed(6))
    );
