...
describe('verified kibo passive generation', () => {
  it('applies damage-triggered defense stacks after the triggering hit and caps at five', () => {
    ...
    expect(laterDefenseTraces.length).toBeGreaterThan(0);
    expect(laterDefenseTraces.every(trace => trace.dynamicPercentRaw < 0)).toBe(
      true
    );
  ...
  it('evaluates OR TargetEntityType conditions before the shared ICD and lifecycle', () => {
    ...
    expect(positive.internalCooldownSuppressions).toEqual([
      expect.objectContaining({
        hitIdentity: 'slime-hit-2',
        internalCooldownMs: expect.any(Number),
        reason: 'kibo-passive-internal-cooldown-active',
      }),
    ]);
    ...
    expect(
      resolveActiveEffectsAt(
        timeline,
        positiveCommand.timeMs + positiveCommand.durationMs - 0.001,
        {
          targetKind: 'enemy',
    ...
    expect(
      resolveActiveEffectsAt(
        timeline,
        positiveCommand.timeMs + positiveCommand.durationMs,
        {
          targetKind: 'enemy',
          targetId: 'enemy-condition-target',
          calculatorOnly: true,
  ...
  it('loads source-verified self properties at scenario start and expires timed ones', () => {
    ...
    expect(
      resolveActiveEffectsAt(
        timeline,
        timedSelfCommand.timeMs + timedSelfCommand.durationMs - 0.001,
        {
          targetKind: 'kibo',
          targetId: 'actor-timed-kibo',
          calculatorOnly: true,
    ...
    expect(
      resolveActiveEffectsAt(
        timeline,
        timedSelfCommand.timeMs + timedSelfCommand.durationMs,
        {
          targetKind: 'kibo',
          targetId: 'actor-timed-kibo',
          calculatorOnly: true,
  ...
  it('applies every root of a timed self passive to the real kibo damage source', () => {
    ...
    const selfAttackTrace = withPassive.damageEvents
      .flatMap(event => event.payload.dynamicPropertyTrace.source)
      .find(trace => trace.attributeId === 1);
    expect(selfAttackTrace).toMatchObject({
      attributeId: 1,
      dynamicPercentRaw: expect.any(Number),
    });
    expect(selfAttackTrace.dynamicPercentRaw).toBeGreaterThan(0);
    const timedSelfCommand = generation.effectCommands.find(
      command =>
        command.effectId === 'kibo-passive:520080:520080001' ||
        command.effectId === 'kibo-passive:520080:520080002'
    );
    expect(
      resolveActiveEffectsAt(
        effectTimeline,
  ...
  it('applies the real PetUltraSkill debuff once, reports later hits, and expires at 40 seconds', () => {
    ...
    const laterDebuffTraces = runtime.damageEvents
      .filter(
        event =>
      ...
      .flatMap(event => event.payload.dynamicPropertyTrace.target)
      .filter(trace => [3, 4].includes(trace.attributeId));
    expect(new Set(laterDebuffTraces.map(trace => trace.attributeId))).toEqual(
      new Set([3, 4])
    );
    expect(laterDebuffTraces.every(trace => trace.dynamicPercentRaw < 0)).toBe(
      true
    );
    expect(
      resolveActiveEffectsAt(
        effectTimeline,
        passiveCommand.timeMs + passiveCommand.durationMs - 0.001,
        {
          targetKind: 'enemy',
          targetId: String(scenario.enemy.id),
          calculatorOnly: true,
    ...
    expect(
      resolveActiveEffectsAt(
        effectTimeline,
        passiveCommand.timeMs + passiveCommand.durationMs,
        {
          targetKind: 'enemy',
          targetId: String(scenario.enemy.id),
          calculatorOnly: true,
  ...
  it('schedules and settles a 5-tick derived dot and self heal after kibo damage for 520092', () => {
    ...
    expect(dotEvents.length).toBeGreaterThan(1);
    expect(healEvents).toHaveLength(dotEvents.length);
    for (let index = 1; index < dotEvents.length; index += 1) {
      expect(dotEvents[index].timeMs).toBeGreaterThan(
        dotEvents[index - 1].timeMs
      );
    }
    for (const event of dotEvents) {
      ...
      const expectedUpper = Math.ceil(event.payload.attack * dotRatio + 1);
    ...
    for (const event of healEvents) {
      ...
      const expectedLower = Math.floor(
        event.payload.sourceAttribute.value * healRatio - 1
      );
  ...
  it('caps BeforeSkill stacks at six, refreshes both targets, and diagnoses non-signature tags', () => {
    ...
    expect(generation.triggerLimitSuppressions).toEqual([]);
    const lastBeforeSkillCommand = generation.effectCommands
      .filter(command => command.effectId === 'kibo-passive:520087:520087002')
      .at(-1);
    const beforeSkillExpiryMs =
      lastBeforeSkillCommand.timeMs + lastBeforeSkillCommand.durationMs;
    expect(
      ...
      { targetKind: 'actor', stacks: 6, expiresAtMs: beforeSkillExpiryMs },
      { targetKind: 'kibo', stacks: 6, expiresAtMs: beforeSkillExpiryMs },
    ]);
    expect(
      resolveActiveEffectsAt(effectTimeline, beforeSkillExpiryMs - 0.001, {
        targetId: baseAction.actorId,
        calculatorOnly: true,
      }).filter(effect => effect.effectId === 'kibo-passive:520087:520087002')
    ).toHaveLength(2);
    expect(
      resolveActiveEffectsAt(effectTimeline, beforeSkillExpiryMs, {
        targetId: baseAction.actorId,
        calculatorOnly: true,
      }).filter(effect => effect.effectId === 'kibo-passive:520087:520087002')
    ).toEqual([]);
  ...
  it('applies every verified damage-triggered root from one hit and caps both at five', () => {
    ...
    const defenseModifierValueRaw = passiveCommands[0].modifiers.find(
      modifier => modifier.attributeId === 3
    ).valueRaw;
    expect(
      Math.min(
        ...laterTargetTrace
    ...
    ).toBe(defenseModifierValueRaw * maxObservedPriorStacks);
    expect(
      Math.min(
        ...laterTargetTrace
  ...
  it('runs a control-reachable static resistance and damage-triggered debuff from one composite passive', () => {
    ...
    const debuffDurationMs = damageCommands[0].durationMs;
    expect(
      resolveActiveEffectsAt(
        effectTimeline,
        refreshedEffectTimeMs + debuffDurationMs - 0.001,
        {
          targetKind: 'enemy',
          targetId: String(scenario.enemy.id),
          calculatorOnly: true,
    ...
    expect(
      resolveActiveEffectsAt(
        effectTimeline,
        refreshedEffectTimeMs + debuffDurationMs,
        {
          targetKind: 'enemy',
          targetId: String(scenario.enemy.id),
          calculatorOnly: true,
    ...
    expect(generation.derivedDamageCommands[0]).toMatchObject({
      ...
      hit: {
        ...
        formula: {
          ...
          ratiosByLevel: expect.objectContaining({ 1: expect.any(Number) }),
        },
        damage: {
          elementalType: expect.any(Number),
          elementalTypeName: 'Fire',
          weakBreakDamageRateBasisPoints: expect.any(Number),
          recoverSp: expect.any(Number),
          petRecoverSp: expect.any(Number),
        },
      },
    });
  ...
  it('consumes a missed derived hit, suppresses a repeat inside two seconds, and allows a later trigger', () => {
    ...
    expect(
      generation.internalCooldownSuppressions.every(
        row =>
          row.skillId === 520041 &&
          typeof row.internalCooldownMs === 'number' &&
  ...
  it('stacks the verified Self attack effect to ten, refreshes it, expires it, and rejects Hero targets', () => {
    ...
    expect(laterDamageTrace.dynamicPercentRaw).toBeGreaterThanOrEqual(
      generation.effectCommands[0].modifiers.find(
        modifier => modifier.attributeId === 1
      ).valueRaw
    );
    const lastCommand = generation.effectCommands.at(-1);
    expect(activeBeforeExpiry).toEqual([
      expect.objectContaining({
        effectId: 'kibo-passive:520090:520090002',
        targetKind: 'kibo',
    ...
    expect(activeBeforeExpiry[0].expiresAtMs).toBeCloseTo(
      lastCommandTimeMs + lastCommand.durationMs,
      6
    );
    expect(
      resolveActiveEffectsAt(
    ...
    expect(commands[0]).toMatchObject({
      ...
      hit: expect.objectContaining({
        ...
        damage: expect.objectContaining({
          ...
          elementalType: expect.any(Number),
          weakBreakDamageRateBasisPoints: expect.any(Number),
          magicRatioBasisPoints: expect.any(Number),
        }),
      }),
    });
  ...
  it('loads every control-map reachable static root without executing orphan asset elements', () => {
    ...
    ).toHaveLength(generation.effectCommands.length);
  });
  ...
  it('rebinds the Thunder condition to each PlayerAllEntity copy and feeds attribute 58 into damage', () => {
    ...
    expect(thunderPropertyTrace.dynamicExtraRaw).toBeGreaterThan(0);
    expect(sumDamage(withPassive, action.id)).toBeGreaterThan(
      sumDamage(withoutPassive, action.id)
    );
