...
describe('M12-B3 visual acceptance pipeline', () => {
  it('publishes and accepts all 254 loadout and Kibo manifests', async () => {
  ...
  it('publishes Kibo manifests with maturity-gated ledgers and pending signoff', async () => {
    ...
    expect(
      kiboManifests.every(
        manifest => manifest.evidence.icon?.status === 'verified'
      )
    ).toBe(true);
    expect(
  ...
  it('closes set-skill 3:4 with its executable static root and scenario boundary', async () => {
    const artifacts = await createVisualAcceptanceArtifacts();
    const entry = artifacts.catalog.entries.find(
      item => item.objectKind === 'set-skill' && item.objectId === '3:4'
    );
    expect(entry.optimizationReady).toBe(true);
    expect(entry.blockers).toEqual([]);
    const manifest = artifacts.manifests.find(
      item =>
        item.owner.objectKind === 'set-skill' && item.owner.objectId === '3:4'
    );
    ...
    expect(
      manifest.matrix.requirements.find(
        requirement =>
          requirement.requirementIdentity === 'set-skill-effect-runtime-applied'
      )
  ...
  it('matches the committed catalog deterministically', async () => {
    ...
    expect(first.catalog.catalogHash).toBe(visualAcceptanceCatalog.catalogHash);
  });
});
