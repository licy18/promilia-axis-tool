...
describe('generated AzPr data', () => {
  ...
  it('keeps the Workbench production projection aligned with full catalogs', () => {
    ...
    expect(
      skillCore.skillLevelCrossCheck.items.filter(item => item.labelIdSeries)
    ).toHaveLength(80);
    expect(
      skillCore.skillLevelCrossCheck.items.filter(item => item.valueIdSeries)
    ).toHaveLength(80);
    expect(
      crossCheckLevels.filter(level => Object.hasOwn(level, 'labelIdRange'))
    ).toHaveLength(40);
    expect(
      crossCheckLevels.filter(level => Object.hasOwn(level, 'valueIdRange'))
    ).toHaveLength(40);
  ...
  it('keeps validation findings explicit for the next reconstruction stage', () => {
    ...
    expect(skillAssetInfo).toMatchObject({
      ...
      summary: {
        ...
        extractedSkillControlDirectories: 4140,
        ...
        hpDamageCandidateSkills: 52,
        toughnessCandidateSkills: 62,
        selfEnergyCandidateSkills: 10,
        behaviorReferenceResolvedSkills: 92,
        hpDamageBehaviorReferenceResolvedSkills: 52,
        externalElementBaseReferenceSkills: 75,
  ...
  it('indexes AzPr Extractor skill control assets as effect-node candidates', () => {
    ...
    expect(evidence.summary).toMatchObject({
      ...
      summonTargetSkillCount: 10,
      summonTargetDamageElementObjects: 11,
      damageElementFieldMappedSkills: 138,
      damageElementFieldMappedObjects: 412,
      hpDamageFieldCandidateRefs: 412,
      toughnessDamageFieldCandidateRefs: 412,
      selfEnergyFieldCandidateRefs: 412,
      damageElementSkillLogicBridgeMatches: 172,
      relationStatus: 'skill-control-assets-found-in-azpr-extractor',
    });
    ...
    expect(evidence.externalElementObjectEvidence).toMatchObject({
      ...
      summary: {
        skillCount: 151,
        resolvedSkills: 151,
        requestedPathIds: 1419,
        resolvedPathIds: 1419,
        unresolvedPathIds: 0,
        formulaParamBuffReferenceObjects: 238,
        formulaParamBuffReferences: 245,
        formulaParamBuffReferenceResolvedObjects: 15,
        unknownScriptBuffReferenceObjects: 108,
        sourceSkillCount: 73,
        targetSkillCount: 79,
      },
    ...
    expect(evidence.summonTargetSkillEvidence).toMatchObject({
      ...
      summary: {
        ...
        summonUnitCount: 6,
        targetSkillCount: 10,
        resolvedTargetSkillCount: 7,
        targetSkillControlStubOnlySkillCount: 0,
        requestedPathIds: 24,
        resolvedPathIds: 24,
        unresolvedPathIds: 0,
        damageElementObjectCount: 11,
        damageElementFieldMappingCount: 11,
      },
    ...
    expect(evidence.damageElementFieldMappingEvidence).toMatchObject({
      ...
      summary: {
        skillCount: 151,
        mappedSkills: 138,
        damageElementObjects: 412,
        hpDamageCandidateRefs: 412,
        toughnessDamageCandidateRefs: 412,
        selfEnergyCandidateRefs: 412,
        skillsubElementBridgeMatchedObjects: 172,
        skillsubElementBridgeMissingObjects: 30,
        skillsubElementBridgeLevelRows: 2064,
        valueParamFormulaSlotDirectMatchObjects: 172,
        valueParamFormulaSlotOverrideCandidateObjects: 172,
        valueParamFormulaSlotUnresolvedObjects: 172,
        formulaFunctionCheckedObjects: 412,
        formulaFunctionDirectElementFormulaObjects: 412,
        formulaFunctionRefs: 824,
        formulaFunctionMatchedRefs: 824,
        formulaFunctionUnmatchedRefs: 0,
        formulaFunctionUniqueIds: [1, 2, 23, 101, 104, 122, 107205],
      },
      skills: expect.arrayContaining([
        expect.objectContaining({
          skillId: 10900101,
