...
export function projectSimulationResult({
  ...
  verifiedNonDamageEventGeneration = null,
  verifiedDamageEventGeneration = null,
  verifiedTuningMarkGeneration = null,
  verifiedPickupEntityGeneration = null,
  ...
  const damageTimeline = damageEvents.map(event => ({
    ...
    requestedHpDamage: event.payload.requestedHpDamage ?? null,
    effectiveHpDamage:
      event.payload.effectiveHpDamage ?? event.payload.rawDamage ?? null,
    overkill: event.payload.overkill ?? 0,
    inBreakForHpDamage: event.payload.inBreakForHpDamage ?? null,
    hpDamageMultiplier: event.payload.hpDamageMultiplier ?? null,
    toughnessBefore: event.payload.toughnessBefore ?? null,
    toughnessAfter: event.payload.toughnessAfter ?? null,
  ...
  return {
    ...
    verifiedKiboPassiveGeneration,
    ...(Number(
      verifiedSoulEssenceEffectGeneration?.summary?.equippedBindingCount
    ) > 0
      ? { verifiedSoulEssenceEffectGeneration }
      : {}),
    ...(Number(verifiedNonDamageEventGeneration?.summary?.eventCount) > 0
