...
`3 角色 + 各自奇波 + 敌人配置 -> 拖入角色/奇波/敌人动作 -> 同轴查看命中与 6 条独立能量/HP/韧性 -> 编辑后即时重算 -> 保存/分享/比较`
...
4. 当前：M1-C 拖入动作到曲线变化的核心闭环；随后完成 M1-D，每个子阶段必须是可见、可操作的完整用户流程。
5. 停止条件：M1-D 验收后停止并报告，不自动展开新里程碑。
