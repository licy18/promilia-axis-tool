...
下一阶段把统一帧游标推进为可操作的时间轴播放闭环：提供播放/暂停、逐帧前后移动、速度控制和循环区段，让当前动作、五条状态曲线及对应日志随游标连续更新。循环范围复用已有 cycle boundary，播放状态只属于瞬态视图，不写入项目载体；继续只消费现有 runtime outputs，不新增公式或未确认机制。
