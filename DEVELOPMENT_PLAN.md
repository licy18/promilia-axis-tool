...
阶段结果：`AzPrMachineAxisCycleDps` 继续由唯一 canonical core 实现。原 R1/R2 的无限 HP、资源、Kibo ICD/finite trigger 与 sampled common-random 约束保留；2026-08-31 将评分闭合从固定两圈/单边界精确相等升级为 `eventual-periodic-operation-and-metrics-v1`。CLI 仍消费同一显式半开循环输入，但自动寻找最终稳定期并按周期平均，不再要求排轴者人为旋转到“干净” Buff 相位。
...
- 当前技术闭合已覆盖统一 normal-input authority、Workbench 单次输入物化、Machine Axis legality、search/cycle/kill pre-score proof、STARBORN genuine context continuation 与 caller marker 防伪；109001、112001 的正向验收轴已全部改成合法完整链。107002 A1/A2 已从既有 MonoBehaviour 命中源补齐真实 projectile impact（A1 两个、A2 八个伤害事务），并以 control/subskill/scenario 精确 source-closure 发布，不再以 `unresolved-action-variant-selection` 进入结构轴；107002 owner matrix 现为 `108/108` required PASS、0 gap/blocker，A1→A4 可正式评分，A5 仍保持 unresolved。当前 verified package hash=`62906a98964fa5948c80519e4454a4c8056f841d620a4c40b959c725f1941fc8`，全包 validator `18/18`、verified-combat clean；错误 subskill、缺段、多义段、伪 context identity继续 fail closed。角色 fixture 与旧视觉记录仍需在最终集成 HEAD 逐一重放和重新签收；在这之前不得启动搜索或把技术测试通过当作角色产品验收。
- 101010 技术验收轴已同步到该 package：孤立 A4 使用完整 A1→A4，charged-input 的 `-200/75/99/100F` 四边界分别验证窗口外默认、窗口内连续重击及右开结束；`74F` source 占轴冲突保留为明确负例。owner matrix 为 `202/202` required PASS、0 gap/blocker，headless/canonical/Workbench 均绿；产品视觉继续 `pending`，未复用旧截图或签收 hash。下一 owner 为 103002 Ruby，必须补齐每次物理输入、context/group 与 E1-E12 消耗链后再做相同聚焦重放。
