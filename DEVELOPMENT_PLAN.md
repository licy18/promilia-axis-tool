...
当前里程碑：**M2 / 三人队配置与时间轴可读性整改已完成并通过产品验收**。角色身份区现只保留五件装备与灵子共 6 个直接装配槽，按 2 行 3 列显示；奇波选择归属各自奇波子轨。统一可视选择器只显示一条可拖动滚动轨，长列表滚轮保持在面板内部。时间轴改为占满首屏剩余高度的固定工作区，放大角色轨、装配图标和 8 条状态曲线后由工作区内部纵向浏览；横向滚动条出现或消失不再改变时间轴外框总高度。动作块按实际持续时间确定宽度，并在轨道中部为上方 Buff 与下方 CD/事件保留空间。既有更新、六轴和五载体回放合同不变，培养效果继续 `unapplied`。下一阶段尚未确认，P3 真实采样保持暂停。
