...
下一阶段：P3 六资源采样导入闭环。让同一批次中的角色 SP 与奇波观测按各自准确动作和资源所有者完成批量绑定，并从一次导入稳定生成 3 条角色能量轴与 3 条奇波能量轴；继续不引入未确认公式、倍率或奇波效果。
...
20. 下一阶段：P3 六资源采样导入闭环，一次导入按准确动作和资源所有者绑定完整六轴观测批次。
