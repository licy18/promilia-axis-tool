...
当前里程碑：**M9-R2 / 全角色派生控制与零时长换人事件（进行中）**。所有剩余多变体动作必须区分输入、资源、状态和自动派生控制源：输入分支由用户显式选择，资源/状态分支由运行时在输入帧自动决定。切人改为精确帧、零时长事件；星携技按已验证的入场/退场逻辑绑定到该事件并生成实际子动作。
...
### M9-R2：全角色派生控制与零时长换人事件（R2A-R2B 已完成，R2C-R2D 进行中）
...
M9-R2A 已固定当前客户端 20 名角色、154 个公开动作引用和 136 个派生 control 的审计分母；63 个多 player/resourceMap control 全部进入 `derivedControlContracts`，未出现静默遗漏。M9-R2B 已将可确认的蓄力档位保存为稳定输入 identity，并由同一 variant runtime 在输入帧先处理资源/状态自动分支、再应用用户输入；检查器选档会同步重建真实时长、命中和效果，自动分支保持只读可溯源。下一步进入 R2C，将切人改为零时长精确帧事件。
...
43. 进行中：M9-R2 全量解析剩余角色派生的输入、资源、状态和自动控制源；输入分支由用户选择，资源/状态分支自动选择。切人改为零时长精确帧标记，星携技按实际入场/退场关系作为其子动作触发。
