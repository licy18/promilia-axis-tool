...
- 下一步：阶段 5-8AX，沿 `DamageElement.RecoverSP`、`RecoverSPArgs..ctor` / `OnReset` 和 `SPSystem.OnTransmit` 追 `TDamageElementParams.recoverSP/petRecoverSP/recoverInterval` 到 `RecoverSPArgs.baseDelta/delta/petDelta/interval` 的构造来源，确认 `recoverTagType` 枚举、共享目标筛选、recoverInterval 时间基准，并设计按角色记录 SP 曲线的 runtime hook 采样点。
