...
当前里程碑：**M9-R1 / 可编排动作与零距离投射物场景整改（已完成，等待产品验收）**。公开动作是否可排轴已与客户端静态证据、当前场景可结算状态分离；来源时长不足时使用明确的规划占轴而不生成伪命中，有可靠发射帧的投射物按零距离场景立即命中。每个 hit 是否命中可独立编辑并确定性回放，场景结果不冒充客户端静态事实。
