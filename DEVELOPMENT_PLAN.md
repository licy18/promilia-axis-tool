...
当前里程碑：**M4 / 约束感知排轴与自动整理实施中，M4-A/B 已完成**。统一放置提议已进入显式“自由 / 约束辅助”模式；自由排轴保持原提交语义，辅助模式在写入历史前整组评估，自动采用最早的确定建议位置，阻塞时不改动作、关系、选中态或剪贴板。下一步进入 M4-C，同轴显示请求帧、建议帧与合法状态，并完成真实拖拽、五载体和视觉发布验收。未知条件只提示，不自动推断。M4 不接入新战斗公式、真实采样、培养效果或分析报告。
...
27. 实施中：M4-A/B 已完成统一合法放置提议与事务式约束编辑。自由模式保留原提交语义；辅助模式只在用户明确选择后整组采用确定建议，阻塞不落盘，关系组、跨轨 owner、剪贴板和撤销/重做保持原子一致。下一步进入 M4-C 同轴反馈与真实工作流验收。
