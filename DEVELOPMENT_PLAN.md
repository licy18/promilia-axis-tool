...
下一阶段：P0/P3 六资源生产试用检查点。把六资源批次打包、一次导入和回放加入候选发布必需能力，运行完整 `test:trial-release`，更新试用手册并重新盘点真实生产样本与未确认机制边界；不新增公式、倍率或碎片 UI。
...
21. 下一阶段：P0/P3 六资源生产试用检查点，把批次导入纳入完整候选发布守门并更新真实样本边界。
