...
下一阶段基于既有 runtime outputs 和统一帧游标，提供全轴或当前 cycle 区段的多维贡献分析：按角色、动作和 HP/韧性/能量三轨聚合，支持从贡献项定位动作或事件并回到编辑。分析只筛选、分组现有 applied transactions，不建立第二套公式，不让候选或未应用装备/奇波/灵子效果混入结果；桌面与窄屏保持时间轴为主舞台。
