...
2026-07-13 主线重定义：Stage 13 及其自动后续路线取消，实测校准 WIP 封存于本地 `deferred/stage-13a-calibration` 分支，M1 完成前不合并。当前唯一产品主线是 **M1 / Endaxis 级核心排轴体验**：让用户无需理解 profile、binding、schema 或校准合同，即可完成 3 人队与奇波配置、多轨编排、五曲线复盘、方案保存/分享/比较。Stage 11/12 分析能力保留为高级二级入口，不再作为主线增长点。
...
`3 角色 + 各自奇波 + 敌人配置 -> 拖入角色/奇波/敌人动作 -> 同轴查看命中与 3 能量/HP/韧性 -> 编辑后即时重算 -> 保存/分享/比较`

#### M1-A：时间轴信息架构与视觉身份

状态：已完成。Workbench 首屏已改为时间轴主舞台：高频命令留在顶栏，项目交换与高级能力进入二级菜单；3 个角色身份、各自奇波子轨、独立能量曲线及敌人事件/HP/韧性在同一坐标内完整显示。检查器默认收起并以覆盖层打开，1440x900 可同屏识别完整拓扑；390x900 使用固定身份列和时间内容局部横向滚动。视觉证据为 `reports/m1a-workbench-desktop.png`、`reports/m1a-workbench-narrow.png` 与 `reports/m1a-endaxis-reference.png`。
...
4. 3 个角色组各包含角色主轴、奇波子轴和能量曲线；敌人组包含事件轴、HP 曲线和韧性曲线。7 条可编排轴与 5 条曲线共享唯一 x 轴。
5. 动作块显示图标/名称、类型或状态、持续时间；命中点、选中态和不可执行态可辨识。曲线平线、阶跃和当前帧值形成清晰层级且不遮挡动作。
6. 1440x900 首屏无需下翻即可识别全部队伍、奇波、敌人和五曲线归属；空方案仍显示全长平线。验收必须与 Endaxis 同尺寸截图对照。

#### M1-B：3 人队与奇波配置闭环

1. 3 个唯一角色槽可换人，并同步头像、动作库、主轴、能量曲线和配置实例。
2. 每个角色可绑定一个奇波，同步奇波身份、动作库和子轴；未确认效果继续 `unapplied`。
...
2. 3 条能量曲线严格按角色隔离；HP 与韧性只在对应 applied event 的准确帧阶跃。
...
| 里程碑 | 用户可完成的事                                 | 验收锚点                                  |
| ------ | ---------------------------------------------- | ----------------------------------------- |
| M1-A   | 首屏直接理解 3 人、3 奇波、敌人、7 轴和 5 曲线 | 1440x900 / 390px 与 Endaxis 对照          |
...
3. 当前：M1-B 3 人队、奇波与敌人配置闭环；随后按 M1-C、M1-D 顺序交付，每个子阶段必须是可见、可操作的完整用户流程。
4. 停止条件：M1-D 验收后停止并报告，不自动展开新里程碑。
