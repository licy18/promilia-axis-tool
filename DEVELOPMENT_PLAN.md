...
下一阶段：P3 首份非 fixture 六资源受控采样验收。由操作者明确启动获准客户端，先执行计划中的一份 `role-sp` 与同槽一份 `kibo-energy`，分别通过 production audit、owner binding 和 Workbench 回放后，再完成其余 2+2；在真实证据到位前不新增公式、倍率或奇波能量推断。
...
25. 下一阶段：P3 首份非 fixture 六资源受控采样验收，从获准客户端的 `role-sp` 与 `kibo-energy` 会话开始，不提前猜机制。
