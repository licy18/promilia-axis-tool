...
下一阶段：P3 六资源所有者机制输入检查点。盘点角色 SP 与奇波终极技能就绪进度中已有稳定 AzPr runtime 来源的事件，只把已确认输入绑定到对应角色或奇波能量轴；没有稳定来源的奇波能量继续 `tracking-only / unapplied`，不猜测 `totalTime`、倍率或平衡数值。
...
18. 下一阶段：P3 六资源所有者机制输入检查点，仅接入已有稳定 AzPr runtime 来源的角色或奇波能量事件。
