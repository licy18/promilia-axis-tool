...
当前里程碑：**M10-B1 / 红宝石星鸣技重导出结算修复已完成，等待产品复验**。上游完整 Typetree 已恢复 `10300212/sub0` 的 7 段命中（37/44/49/54/59/64/69F）及 0F 补弹、火印记和强化入口；公开形态覆盖与逐 hit 三值已接入。红宝石保持 `runtime-applied / partial`，零距离模拟完整、实机证据仍不完整。M11、下一角色与包体优化继续暂停。
...
- Discovery 与阶段审计当前得到 10 个公开动作、27 个 control、24 个执行形态、225 个命中，以及普通 A1-A3 和强化 E1-E12 两个 applied 输入阶段；重导出恢复星鸣技终端链后，公开形态门禁达到 10/10 runtime-ready。角色总 Profile 仍因 4 项实机 capture 与静态效果缺口保持 `partial`。
...
- 重导出确认旧缺口来自 `--stub-types monobehaviour`：原始 bundle 与完整 Typetree 均保有终端玩法图。同步器现通用遍历外部 Hero SubSkill 引用，恢复 `10300212/sub0` 的 7 段真实命中；每段 `weakBreak=7000`、`recoverSP=3000`、`petRecoverSP=11900`、`recoverInterval=9999`，首段使用元素 `103002247`，后六段使用 `103002248`。覆盖继续按 `public action form + subskill + description clause` 独立结算，星携合击 `10300226` 不再代偿星鸣技伤害。红宝石现为 `10/10 runtime-ready`、`zero-distance-simulation-complete=true`，但仍有 4 项实机 capture 与 429 条静态证据缺口，因此保持 `runtime-applied / partial`、`characterComplete=false`。
