...
当前里程碑：**M4 / 约束感知排轴与自动整理待实施**。M3 已完成并通过产品验收；下一步把现有动作占用、CD readiness、受控角色区间、动作关系和时间边界规则前移到统一放置提议，使动作库拖入、已有动作移动、粘贴和跨轨批量编辑在提交前即可看到合法、可自动调整或确定阻塞的结果。M4 复用现有 compiler、execution plan 和 diagnostics，不建立第二套排轴规则；自由排轴继续允许保留冲突用于诊断，约束辅助只能在用户明确选择时提交建议位置。未知条件只提示，不自动推断。M4 不接入新战斗公式、真实采样、培养效果或分析报告。
...
27. 待实施：M4 约束感知排轴与自动整理。按统一合法放置提议、事务式约束编辑、同轴拖拽反馈三个大验收块，把现有占用/CD/关系诊断前移到动作库拖入、移动、粘贴与批量编辑；保留自由排轴，不新增求解器、公式、真实采样或碎片 UI。
