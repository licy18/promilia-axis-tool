...
下一阶段：动作/效果关系语义闭环。在现有 `sequence` 关系之上定义动作触发、效果创建/刷新/消耗及来源回溯的统一合同，并贯通编辑、runtime 与项目回放；优先使用已确认的现有状态效果数据，不新增伤害公式或测试期数值推断。
...
17. 下一阶段：动作/效果关系语义闭环，把触发、创建/刷新/消耗与来源回溯接入统一编辑和 runtime 合同。
