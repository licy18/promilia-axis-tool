...
#### M12-B2：可持续循环 DPS（R1 整改完成，等待产品复验）
...
- 默认纯伤害暴击使用 `expected`；存在状态型暴击副作用时继续要求精确带权分支或显式多 seed `sampled`，报告样本统计，不能用单 seed 冒充循环期望。
...
阶段结果：`AzPrMachineAxisCycleDps` v1 已通过唯一 canonical core 实现，CLI `cycle` 可重放显式循环并输出暖机、半开区间伤害、三层贡献、资源/状态差、两轮可执行性、动作形态闭环、假设和 hash。R1 进一步保证无限 HP 模式在普通、调谐/超限与真实伤害公式入口不受初始有限 HP 的 minimum-HP 截断，同时继续保留护盾结算；奇波被动的内置 CD 与有限触发寿命已成为 canonical 显式状态并按边界相对余量闭环。权威 5 秒红宝石 A1-A3 示例仍为每轮 `22.44996643` 伤害、4 hit，`cycleDps=4.48999329`；驮驮龙 15 秒内置 CD 的 5 秒伪循环现会被拒绝。当前停在 M12-B2-R1 产品复验点，M12-C 继续锁定。
...
66. 已通过产品验收：M12-B-R2 在 `e3777ecf6ae4564a4164e2fd567ff058aa80b9ac` 关闭可行动资源阈值与多 seed 贡献守恒两项缺口；M12-B 正式关闭。M12-B2-R1 已关闭无限 HP 公式封顶与奇波被动内置 CD/触发寿命未入闭环两项阻断，当前等待产品复验；M12-C 继续锁定。
