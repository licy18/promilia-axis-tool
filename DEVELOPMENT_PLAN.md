...
阶段结果：`AzPrMachineAxisCycleDps` v1 已通过唯一 canonical core 实现，CLI `cycle` 可重放显式循环并输出暖机、半开区间伤害、三层贡献、资源/状态差、两轮可执行性、动作形态闭环、假设和 hash。权威 5 秒红宝石 A1-A3 示例两轮均为 `22.44996643` 伤害、4 hit，`cycleDps=4.48999329`；资源净亏、第二轮 CD、一次性 Buff、空循环和延迟 hit 边界均有阻断回归。调谐印记按层数不劣、共享 20 秒衰减余量不退化及独立 5 秒就绪等待不增加判定，不再对层数与计时器做整行精确哈希。当前停在 M12-B2 产品验收点，M12-C 继续锁定。
...
66. 已通过产品验收：M12-B-R2 在 `e3777ecf6ae4564a4164e2fd567ff058aa80b9ac` 关闭可行动资源阈值与多 seed 贡献守恒两项缺口；M12-B 正式关闭。M12-B2 可持续循环 DPS 已实现并等待产品复验，M12-C 继续锁定。
