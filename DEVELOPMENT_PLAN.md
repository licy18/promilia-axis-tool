...
当前里程碑：**M2 / 三人队配置可用性滚动缺陷整改完成，等待重新验收**。时间轴身份区现已提供角色、奇波、五件装备、灵子和敌人的直接装配入口；统一可视选择器按需加载真实目录，长候选列表在面板内部滚动并显示常驻滚动位置，后台工作台在选择器打开时保持锁定。既有更新、六轴和五载体回放合同不变，培养效果继续 `unapplied`。M2 完成后停止扩展，P3 真实采样保持暂停。

### M2-R1：时间轴直接装配表面（已完成）

- 在现有角色身份列内提供独立头像按钮和 7 个固定尺寸槽位：奇波、武器、上装、下装、耳环、戒指、灵子；身份容器不再伪装成包裹按钮的 `role=button`，槽位事件与轨道选择、游标和旧检查器隔离。
...
- 奇波、装备、灵子和敌人等长列表使用面板内独立滚动区和常驻滚动轨；鼠标滚轮、触控滚动及列表顶/底的滚动链不得带动后台工作台。
...
验收结果：93 个测试文件、508 条单元测试，41/41 production preview 与 40/40 Workbench 主流程 E2E 全部通过；production M2 路径额外断言奇波/灵子列表真实滚动、滚动滑块同步和后台页面位置不变。生产导入、Workbench 数据和 applied-source 审计通过。总 JavaScript gzip 为 719,622B，初始入口 89,377B，Workbench 主块 329,032B，统一选择器保持为 3,099B lazy chunk。桌面与窄屏滚动证据为 `m2-loadout-picker-scroll-desktop.png` 和 `m2-loadout-picker-scroll-narrow.png`，下一步仅等待产品重新验收。
