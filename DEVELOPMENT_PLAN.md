...
下一阶段：M1 空方案从零编排验收。从新建空方案开始配置 3 个角色、各自奇波和敌人，再拖入角色/奇波/敌人动作、复盘八条状态曲线并完成保存恢复；只处理这条完整任务的实际阻塞，不新增公式、校准、报告合同或碎片 UI。
...
13. 下一阶段：执行 M1 空方案从零编排验收，确认核心体验不依赖内置示例数据，只修复从建队到项目恢复的完整流程阻塞项。
