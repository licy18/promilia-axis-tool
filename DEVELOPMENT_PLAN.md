...
下一阶段先执行 P0 工程收口：把全部 JavaScript gzip 从当前 738,352B 恢复到至少 735,000B 以下，并对 30 秒及更长轴的连续播放、缩放自动滚动、暂停响应和卸载清理建立性能守门。该阶段只做已有能力的生产优化和合同去重，不新增公式、机制、碎片 UI 或新的持久化字段；余量恢复后再选择下一块 Endaxis 功能差距。
