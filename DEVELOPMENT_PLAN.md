...
下一阶段：P3 首份非 fixture 六资源受控采样验收。由操作者明确启动获准客户端，分别以 `role-sp` 与 `kibo-energy` 取得首份真实会话并通过 `--require-production`、owner 绑定和项目回放，再扩展到完整 3+3 批次；在真实证据到位前不新增公式、倍率或奇波能量推断。
...
23. 下一阶段：P3 首份非 fixture 六资源受控采样验收，从获准客户端的 `role-sp` 与 `kibo-energy` 会话开始，不提前猜机制。
