...
下一阶段：M1 试用候选发布 checkpoint。冻结当前从示例和空方案出发的两条主流程基线，整理可试用入口、已知限制和真实用户反馈通道；只处理会阻断完整排轴任务的问题，不新增公式、校准、报告合同或碎片 UI。
...
14. 下一阶段：M1 试用候选发布 checkpoint，冻结核心排轴基线并转入真实试用反馈，不再自然延伸新的局部交互或公式阶段。
