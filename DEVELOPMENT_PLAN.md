...
当前里程碑：**M5 / 可复用编排片段与动作组模板，待实施**。M4 已通过产品验收，下一步把当前时间轴中已经排好的局部动作组沉淀为可复用片段，并从动作库直接拖回主轴。片段只保存动作来源、固定队伍槽兼容要求、相对帧差、轨道归属和关系语义；插入时重新经过 M3 generation/runtime 与 M4 放置提议，不保存旧 CD、Buff、曲线或日志结果。M5 不做自动最优循环、数值评分、真实采样、公式研究或社区模板平台。
...
28. 待实施：M5-A/B/C 建立局部编排片段合同、复用 M4 的原子放置并在动作库提供可拖拽片段视图。片段插入后必须重建 M3 runtime，禁止保存旧结果或猜测不兼容 owner；阶段结束前不进入自动最优循环、公式研究、真实采样或社区平台。
