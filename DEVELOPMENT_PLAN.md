...
下一阶段：M1 试用工作流 checkpoint。以默认方案完整走查“编排 -> 运行 -> 事件/曲线定位 -> 三值复盘 -> 回源编辑 -> 五载体恢复”，只处理一个最大的真实流程阻塞项，不开展公式、校准、报告或碎片 UI 阶段。
...
10. 下一阶段：执行 M1 试用工作流 checkpoint，以真实用户路径选出一个最大的完整流程阻塞项；不新增公式、校准、报告载体或碎片 UI。
