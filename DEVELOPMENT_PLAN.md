...
下一阶段把单动作多轨编辑扩展为可用于实际排轴的批量工作流：框选应能跨角色动作轨、奇波轨和敌人轨形成混合选择；批量移动、复制、删除和撤销/重做必须保持各动作的合法轨道、角色/奇波归属、动作关系与 runtime 节点。30 秒及更长时间轴下，批量操作、缩放、横向滚动和五载体回放仍需保持动作、曲线与状态点对齐。该阶段不接入奇波效果或新公式，也不拆成单个按钮或提示任务。
