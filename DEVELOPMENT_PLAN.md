...
当前里程碑：**M7 / 全角色、全技能三值变化数据映射与运行时覆盖已完成，等待产品验收**。当前客户端公开角色/奇波动作目录已形成可重复、可审计的 `owner -> public skill/action variant -> skill_control -> frame trigger -> Battle element -> HP/韧性/角色 SP/奇波 SP` 映射；M6-R2 的 `0..100` 绝对 SP 合同保持不变。M7 验收前不自动进入下一里程碑。
