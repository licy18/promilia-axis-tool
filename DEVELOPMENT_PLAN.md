...
#### M12-B2：可持续循环 DPS（R2 验收后的下一优先级）

- M12-B-R2 通过产品复验后，优先实现 `cycle-dps`，早于 M12-C、进一步性能优化和其他评分扩展。
...
#### M12-B-R2 正确性收口完成，等待产品复验（2026-08-01）

M12-B-R1 已关闭原五项主体缺口；R2 进一步把资源等待从逐整数事件改为候选动作真实费用驱动的最早可执行阈值，并让多 seed 的角色、动作与 hit 贡献按稳定 identity 聚合且与排序指标守恒。SP 90→100 与奇波能量 99→100 均由真实增长轨迹产生直接阈值等待，无增长的特殊资源不会伪造等待；405/425 两个样本的总指标和三层贡献均为 415。当前停在 R2 产品复验点，M12-B2 与 M12-C 均未启动。
...
66. 等待产品复验：M12-B-R2 已关闭可行动资源阈值与多 seed 贡献守恒两项缺口；M12-B2、M12-C 均未启动。
