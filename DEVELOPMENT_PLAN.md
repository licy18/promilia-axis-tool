...
当前里程碑：**M9-R3-R1 / 时间轴布局修复（已完成，等待产品验收）**。队伍共享印记组已整体移至所有角色之前，底部以独立 `20px` 尾槽隔离水平滚动条且 PNG 导出不保留交互空槽；业务 lane identity、事件与因果关系未改变。
...
默认普攻链现使用 `20/35/47/30/80F` 的实际输入占轴，A5 不再以 `240F` 完整动画尾长占轴；爆发状态普攻切换为 `72/75/72F` 三段链。A5 后重击在来源窗口内选择特殊重击 subskill 1，爆发链对应窗口选择强化特殊重击 subskill 2，窗口外维持普通规则；辅助排轴吸附到最早合法帧，运行时以整数帧判断边界。缘结值同一交易跨至 100 时清空并进入 10 秒状态，星决技在来源 `272F` 进入或刷新状态；状态同时驱动资源轴、统一 Buff 区间、普攻链和重击形态。
