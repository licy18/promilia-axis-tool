...
下一阶段把现有 A/B 方案比较与 Stage 11-A 的时间窗口合同打通：用户可在全轴或同一 cycle 区段按角色、动作及 HP/韧性/能量比较两条方案，并从差异项分别定位当前方案和基准方案来源。比较继续只消费两份独立 runtime outputs，不复制公式，不混入 candidate/unapplied 来源，也不拆成零散统计标签。
