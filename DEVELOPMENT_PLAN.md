...
#### M12-B2：可持续循环 DPS（下一优先级）

- M12-B-R1 搜索正确性整改完成后，优先实现 `cycle-dps`，早于 M12-C、进一步性能优化和其他评分扩展。
...
#### M12-B-R1 正确性整改完成，等待产品复验（2026-08-01）

M12-B-R1 已修复重复动作 ID、dead-end 丢失、错误节点状态、事件边界未接主循环、burst 窗口评分偏差、暴击 seed 安全门和跨队伍枚举缺口。真实 CLI 深度 8/24 均返回同深度数量的唯一动作，1 秒 burst 的排序分与报告一致，两个队伍候选进入同一 Top-N；当前等待产品复验。下一优先级仍是 M12-B2 可持续循环 DPS，M12-C 与正式最优结论继续锁定。
