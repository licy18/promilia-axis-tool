...
当前里程碑：**M2 / 三人队配置可用性二次整改完成，等待重新验收**。角色身份区现只保留五件装备与灵子共 6 个直接装配槽，按 2 行 3 列显示；奇波选择归属各自奇波子轨。统一可视选择器只显示一条可拖动滚动轨，长列表滚轮保持在面板内部。角色动作轨与图标已加高放大，动作块宽度严格使用实际持续时间，短动作仍保留稳定拖拽热区。既有更新、六轴和五载体回放合同不变，培养效果继续 `unapplied`。下一步仅等待产品重新验收，P3 真实采样保持暂停。
...
- 1440x900 同屏保留三角色、三奇波、敌人与槽位；390x900 关闭选择器后回到完整无横向溢出的时间轴。截图至少覆盖桌面选择器、桌面装配完成、窄屏选择器、窄屏关闭后时间轴。
- 角色动作轨加高到可清楚辨识头像与 6 个装配图标，状态曲线保持紧凑以维持 1440x900 完整拓扑；动作块和关系线按 `durationMs / axisDurationMs` 使用真实宽度，不再设置 8% 展示下限。短动作通过容器宽度收起内部编辑手柄，使移动、跨轨和批量拖拽仍可操作。
...
验收结果：93 个测试文件、508 条单元/组件测试与 41/41 production preview 全部通过；production M2 路径断言只有一条可见滚动轨、原生滚动条隐藏、奇波/灵子列表真实滚动、滚动滑块同步和后台页面位置不变，并按像素验证动作宽度与实际持续时间一致。生产导入、Workbench 数据和 applied-source 审计通过。总 JavaScript gzip 为 719,863B，初始入口 89,375B，Workbench 主块 329,063B，统一选择器保持为 3,327B lazy chunk。桌面与窄屏证据为 `m2-loadout-picker-scroll-desktop.png`、`m2-direct-equipped-desktop.png`、`m2-loadout-picker-scroll-narrow.png` 和 `m2-direct-timeline-narrow.png`，下一步仅等待产品重新验收。
...
25. 整改中：M2 时间轴直接装配。角色头像、奇波子轨和敌人身份必须成为可视选择入口，角色身份区直接显示奇波/五件装备/灵子槽位；E2E 必须通过卡片选择器完成换人和装配，不能调用右侧原生下拉框冒充主流程。
