...
下一阶段：M1 试用发布主流程验收。以默认示例完成一次“换人/绑奇波/选敌人 -> 拖入动作 -> 同轴复盘 -> 回源编辑 -> 保存与项目恢复”的真实浏览器走查，只处理仍会阻断完整试用任务的问题，不新增公式、校准、报告合同或碎片 UI。
...
12. 下一阶段：执行 M1 试用发布主流程验收，走通配置、编排、同轴复盘、回改和项目恢复，只修复完整流程中的实际阻塞项。
