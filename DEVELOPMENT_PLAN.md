...
### M9-R2：全角色派生控制与零时长换人事件（R2A 已完成，R2B-R2D 进行中）
...
M9-R2A 已固定当前客户端 20 名角色、154 个公开动作引用和 136 个派生 control 的审计分母；63 个多 player/resourceMap control 全部进入 `derivedControlContracts`，未出现静默遗漏。控制源、输入关系、资源/状态条件、默认来源与动作实例实际选择已分离，EventBridge 输入窗口不再被误报为自动后续。下一步进入 R2B，将可确认的用户输入选择和资源/状态自动决策接入动作实例、运行时与检查器。
