...
下一阶段：M1 动作事件定位与复盘可读闭环。让动作块、命中节点、资源节点和状态曲线断点在同一 60fps 坐标中共享清楚的动作来源与选中关系，用户可从任一节点定位到对应动作、准确帧和三值详情，再回到动作修改并看到同步刷新；不新增公式、校准或碎片状态提示。
...
9. 下一阶段：完成动作事件定位与复盘可读闭环，使动作块、命中/资源节点、曲线断点和检查器共享动作来源与准确帧定位；不接入新公式或拆分碎片 UI 阶段。
