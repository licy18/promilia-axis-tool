...
下一阶段把当前静态曲线与日志联动升级为时间轴内的逐帧复盘：动作、命中节点、五条状态曲线和日志共用一个 60fps 帧游标；点击时间网格、曲线断点或日志应定位同一帧，并读取该帧的 3 条角色能量、敌人 HP、敌人韧性与正在发生的动作/事件。缩放和横向滚动后游标仍需像素对齐，编辑动作后当前帧状态响应式刷新。该阶段只消费现有 runtime outputs，不新增公式、候选值应用或碎片提示。
