...
当前里程碑：**M9 / 全动作时序、派生与角色资源机制收口（产品目标 2026-07-21 修订，M9-A 已完成，下一阶段 M9-B）**。M8-A 至 M8-D 已完成并通过产品验收，培养装配、角色到奇波继承、动态效果、九元素队伍印记和同轴溯源主链成立。M9 先修正全动作占轴时长，并建立由角色特殊资源驱动的派生动作状态机，再收口效果目标、触发、包装与公式；未解析时长不得伪装为一帧动作。
...
39. 下一阶段：M9-B 派生动作与角色特殊资源状态机。建立通用变体图和资源合同，用红宝石、涂山小玉验收资源获取/消耗驱动的派生选择及独立资源轴。
40. 已定义：M9-C 效果语义、目标/触发与公式收口。保留当前已做的枚举分流与语义投影，待动作时长和实际变体成立后接入唯一 effect runtime 与 Q16.16 注册表。
41. 已定义：M9-D 公开动作可运行覆盖与真实队伍验收。以 562 动作、20 角色、122 奇波为产品分母，收口所有静态可解项并通过包含红宝石、涂山小玉的连续因果链、五载体和完整发布守门。
