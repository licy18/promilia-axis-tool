...
当前里程碑：**M10-B2-R1 / 寒悠悠产品复验整改已完成实施，等待产品复验**。公共入口已锁定焰火叠层/消费、条件爆炸、Buff/SP、星决技双 Buff 和正负例的同源因果；角色仍诚实保持 `runtime-applied / partial`，未达到 `visually-accepted`，不自动启动 M11 或下一角色。
...
- 通用 compiler v3 新增声明式敌方目标状态、逐命中状态交易、条件命中组和运行时效果绑定。寒悠悠 `焰火` 使用 `element 101003079`、容量 15、逐层 10 秒；星鸣技七次真实命中逐次叠层，被动 `10100362` 在末次命中追加一层。重击一段按 `>=6` 消耗 6 层、否则全消；重击二段按 `>=10 / >=8 / 否则全消` 结算，并在来源帧增加 1 枚火属性调谐印记。
...
- 120 秒 authoritative golden 以真实 compiler + simulation 生成 61 条断言：寒悠悠总 HP/韧性变化为 `69,372 / 6,069`，两次爆炸共直接回复 4 SP，动态攻击与调谐、火印记、前后台角色/奇波 SP 及装配传播均有精确结果；golden hash 为 `c6e72904bfb5ba21a83568527f203320c1344be4c4fa056cb976573769501650`。
- 真实 Workbench 已覆盖公共入口拖入星鸣技与重击、逐 hit 开关、无焰火负例、撤销/重做、刷新回载和星决技双 Buff；1440x900 和 390x900 均无新增溢出。140 个测试文件/846 条测试、62/62 production preview、41/41 必需能力以及 character/verified、production imports、Workbench data、action status、applied-source 守门均通过。
- 角色状态仍为 `runtime-applied / partial`、`characterComplete=false`：42 条 semantic unresolved 中有 11 项 runtime evidence、31 项 static evidence gap，其中 25 项影响玩法结果。Workbench gzip `368,459B` 低于 370,000B；总 JS gzip `757,854B` 超 740,000B 硬门槛 `17,854B`，本阶段只记录独立发布风险，不转去压包或启动下一角色。
...
- R1 已从公共动作入口锁定完整因果链：星鸣技七段命中与被动形成焰火 `0 -> 8`，重击一段消费 6 层并物化 5 次爆炸，检查器和 HP/韧性节点明确区分 4 次本体命中与 5 次条件命中；关闭一段 E hit 得到 `7 -> 1`，移走 E 后只保留 4 次本体命中，撤销和保存重放恢复同一结果。
...
#### M11-A：Canonical Headless Combat Core

- 定义无 UI 的纯接口，至少覆盖 `catalog`、`compile`、`validate`、`simulate`、`evaluate` 和 `explain`。
- 输入包含数据/profile 版本、队伍与装配、敌人、初始状态、战斗时长和语义动作意图；输出包含归一执行计划、合法性诊断、逐帧事件、状态快照、三值/资源/Buff 变化、贡献拆分、因果链及输入哈希。
...
验收门：不启动浏览器即可完成确定性模拟；输入顺序、同帧优先级、状态到期和派生结果有精确断言；三条金标准在迁移前后无语义漂移。

#### M11-B：Machine Axis Contract 与 CLI

- 建立版本化 JSON Schema，支持绝对帧、`after previous/end`、偏移、切人、用户可选 variant、hit mask、初始资源、敌人参数和目标时长。
...
验收门：一条命令可完成完整长轴模拟、对比和解释；保存重放稳定；任何结果都能回到具体 action/hit/effect 与来源版本。
...
验收门：人可以从页面直接发现条件未生效、派生错形态、Buff 漏失、同帧顺序错误和 hit 未结算；页面每个可见事实都能映射到 trace identity。
...
验收门：每个 `optimization-ready` 角色都有机器断言、可视场景和产品验收记录；任一门失败都会撤销优化资格。
...
- 输出带场景假设、数据版本、合法性、覆盖可信度和因果解释的 Top-N 结果，不给脱离前提的单一“最佳答案”。

#### M12-C：末音试点

- 先完整验收末音和一小组候选队友，不等待全角色完成。
- 固定等级/技能/灵子/装备、敌人、防御/韧性、初始能量、战斗时长和目标函数，分别搜索持续输出、短时爆发与削韧方案。
...
52. 已完成并通过产品验收：M10-B1-R3 已完成红宝石 10 个公开动作、146 条原始窗口和 53/53 条语义转移的衔接/派生闭包，接通 A3、换弹、星鸣技、星决技、入场星携技五种强化入口及闪击续段、E12/空弹/超时/切出重置；独立复验确认真实切人、54F 雷印记、公开普攻拖入 E1-E12、跨窗回退、历史与回载均正确。红宝石总 Profile 仍为 `runtime-applied / partial`，保留 5 个 runtime capture 和静态机制缺口；当前停在阶段边界，不启动下一角色。
...
55. 实施完成，等待产品复验：M10-B2-R1 已用公共入口和强正负例锁定寒悠悠焰火 `8 -> 2`、4+5 次命中、引爆 Buff/SP、星决技 24 秒全队双层与 15 秒主控比例 Buff；140 文件/846 测试、62/62 production preview 与全部漂移/来源守门通过。角色仍为 `runtime-applied / partial`，当前不启动 M11 或下一角色。
