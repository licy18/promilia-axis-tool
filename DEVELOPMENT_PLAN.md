...
当前里程碑：**M3 / 真实动作状态生成闭环待实施**。M2 已完成并通过产品验收；下一步复用现有 readiness timeline、effect runtime、interval projection 和 action/effect relation graph，把真实动作目录接入统一生成与运行时链路，使拖入动作后自动形成有可靠来源的 Buff/状态区间、CD 窗口和生命周期节点。缺少可信来源时保持空区或平线，未知效果继续 `tracking-only / unapplied`。M3 不接入测试期倍率、装备/灵子/奇波培养公式、防御、抗性或真实采样，不扩张分析报告，也不拆成零碎 UI 阶段。
...
26. 待实施：M3 真实动作状态生成闭环。先审计真实动作目录的可信 CD/效果覆盖，再按 generation、runtime、同轴 UI 三个大验收块贯通自动生成、生命周期重算、五载体和真实用户流程；缺少来源时保持空白，不启动数值考据或 P3 真实采样。
