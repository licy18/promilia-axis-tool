...
下一阶段：受控角色与切人编排闭环。把现有 `switch` 动作从单条日志提升为按帧维护的受控角色区间，贯通时间轴、runtime、诊断、循环继承、撤销/重做和五载体回放；不新增伤害公式，也不扩张分析报告。
...
16. 下一阶段：受控角色与切人编排闭环，让切人动作在准确帧改变正式受控角色状态，并在编辑、运行时与五载体回放中保持一致。
