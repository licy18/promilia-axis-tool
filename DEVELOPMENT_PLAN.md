...
当前里程碑：**M5 / 可复用编排片段与动作组模板，实施中（M5-A/B 已完成，下一步 M5-C）**。局部片段已具备独立合同、本地库、严格兼容检查及事务式插入路径；下一步只完成动作库可见的“动作 / 片段”工作流、真实拖拽、桌面/窄屏与完整发布验收。片段不保存旧 CD、Buff、曲线或日志结果。M5 不做自动最优循环、数值评分、真实采样、公式研究或社区模板平台。
...
28. 实施中：M5-A/B 已完成局部片段合同、独立本地库、严格兼容检查、M4 整组放置和 M3 运行时重建；下一步 M5-C 在现有动作库交付保存、搜索、导入导出、点击/拖拽插入和真实工作流验收。禁止保存旧结果或猜测不兼容 owner；阶段结束前不进入自动最优循环、公式研究、真实采样或社区平台。
