...
当前里程碑：**M8 / 实际战斗数值机制全量接入（M8-A/B/C 已完成，下一步 M8-D）**。M6-R 至 M7-R3 已完成产品验收；M8-A 已接通培养项到角色/奇波静态属性及下游动作输入，M8-B 已接通全动作 Battle 效果图、动态属性生命周期和逐维覆盖审计，M8-C 已接通队伍印记获取、逐层到期、消费、九属性持有/超限结算与循环继承。Word 报告只供人读，生成与运行时必须消费知识库结构化快照及原始 Battle/NewTable 配置，不从技能描述文本猜数值。
...
37. 下一步：M8-D 同轴 UI、数值溯源与全量验收。印记资源轴、效果轴和装配/动作检查器共享同一 runtime 事实，五载体和完整发布守门通过后等待产品验收。
