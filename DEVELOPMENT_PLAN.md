...
下一阶段在不改变 Stage 11-A/B 结果和交互的前提下，把总 JS gzip 从接近 735,000B 的边界恢复到约 733,000B，并建立双 120 动作方案的窗口切换、贡献差异、来源定位和卸载清理守门。优先合并比较/窗口投影的重复字段与按需计算边界，不放宽预算，不新增公式、机制或碎片 UI；余量恢复后再进入下一块 Endaxis 分析能力。
