...
当前里程碑：**M6-R2 / SP 单位契约修正已完成，等待产品复验**。角色、奇波、事件、曲线、诊断与五载体现统一使用 `0..effectiveMaxSp` 的绝对 SP 点；`spCost=100` 是绝对消耗，基础 `MAXSP=1` 经成长模板倍率 `100` 得到 `effectiveMaxSp=100`。当前停在 M6-R2，不扩机制覆盖，也不进入 M7。
