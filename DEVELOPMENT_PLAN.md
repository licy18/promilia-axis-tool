...
- B3-A 基线已由唯一生成命令重算为 `11/43/62/137/12`，source/roster/manifests/ledger/catalog/binding hash 分别为 `0e2fa93b88d82707`、`6b5961e83e5b1eb7`、`18f56af7088c1225`、`d95dd2bdd11ecf54`、`a97ba5f4ff6a397e`、`3cf1c9cc678e4193`。版本化培养 profile 已进入 Machine Axis、canonical input hash 与 Workbench round-trip；角色等级、星赐层与所选当前层节点的静态属性、六档临阶静态属性、奇波等级/四维/羁绊、灵子等级/突破、装备强化与来源明确的同调公式现投影到唯一静态属性编译链，未使用该合同的旧轴 hash 不漂移。星赐节点技能等级与临阶技能解锁保持明确未应用，研究模式保持兼容，正式模式对当前未取得资格的对象在 project 生成前结构化拒绝。当前五类 optimization-ready 均为 0，DNA、灵子升星动态、装备实例档、`62/62` 灵子效果和 `12/12` 套装技能继续硬阻断，不构成部分放行。
