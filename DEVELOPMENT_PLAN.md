...
当前里程碑：**M5 / 可复用编排片段与动作组模板已完成，等待产品验收**。用户已可把当前多选动作或完整关系组保存为独立局部片段，并在身份兼容的方案中通过点击或真实拖拽整体插回时间轴；插入继续复用 M4 放置提议与 M3 generation/runtime 重建，不保存旧 CD、Buff、曲线或日志结果。M5 不做自动最优循环、数值评分、真实采样、公式研究或社区模板平台，验收前不自动进入 M6。
...
### M5-C：时间轴片段库与真实工作流验收（已完成，等待产品验收）
...
28. 已完成，等待产品验收：M5-A/B/C 已交付局部片段合同、独立本地库、严格兼容检查、M4 整组放置、M3 运行时重建，以及动作库中的保存、搜索、导入导出、点击/拖拽插入工作流。片段不保存旧结果、不猜测不兼容 owner；当前停在 M5 验收点，不自动进入 M6、自动最优循环、公式研究、真实采样或社区平台。
