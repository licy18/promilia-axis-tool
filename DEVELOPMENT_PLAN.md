...
下一阶段让当前可见拓扑成为完整可编辑工作区：动作或事件可从统一动作源拖入合法的角色动作轨、关联奇波轨和敌人事件轨，并在轨内完成移动、复制、删除、框选和撤销/重做；项目交换、runtime 重算和五条曲线必须同步保持一致。奇波与敌人事件只使用已确认的事件/来源合同，未确认效果继续 `unapplied`，不把单个入口、标签或样式当作阶段交付。
