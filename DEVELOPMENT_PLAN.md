...
当前里程碑：**M10-A / 单角色战斗运行逻辑解析与接入流水线（实现已完成，等待产品验收）**。现已可用同一入口按 `ownerId` 发现可达战斗图、生成版本化角色 profile、编译现有 verified runtime 合同、输出缺口/采样计划与 golden trace；涂山小玉 `101010` 为首个金标准，M10-B 在本轮验收前不得启动。
...
#### M10-A：流水线与涂山小玉金标准（实现已完成，等待产品验收）

- 已提供 profile schema、`--owner 101010` 单角色同步、全量漂移审计、20 角色 coverage queue，以及 source manifest、reachable graph、description/runtime coverage、unresolved ledger、capture plan 和 120 秒三人队 golden trace。
- 小玉固定分母为 10 个公开动作、21 个执行形态、20 个可达 control、86 条 verified window 和 108 个 hit；普通/爆发普攻链、四种重击与连续重击、7 条隐藏输入边、缘结值/爆发状态、被动 `10101061`、动态属性、切人/星携技和三值/SP 均由同一 profile/runtime 输出追踪。`10101062` 继续按当前客户端未实装标为 `not-applicable`。
- 单角色与全量同步重复运行均为零漂移；owner 成功审计和非法 owner 失败均不会改写全量 verified package。生产 runtime、Workbench 与 UI 没有新增 `101010` 分支；同步脚本中仍保留的小玉专用函数只承担 Battle 证据适配，后续角色接入不得复制这些函数，而应扩充通用 Discovery 算子。
- 当前明确剩余 4 条 `10101042` 投射物命中需要运行时证据；370 条静态缺口记录主要来自 raw wrapper、目标/触发和非字面公式边，保留来源 identity，不因描述已覆盖而伪装为 applied。
- 133 个测试文件、775 条完整单测及小玉 3/3 production preview 通过；生产引用、Workbench 数据、动作状态、verified 漂移和 applied-source 守门通过。全历史 production preview 为 37/57，20 条失败集中在 M9-R3-R3 后按需检查器/复盘面板的旧 E2E 定位假设，M10-A 未改对应 UI，列为独立测试维护风险。Workbench gzip `361,658B` 通过主块门槛；总 JS gzip `746,688B`，较 `c8c32b5` 增加 `6,243B`、超发布硬门槛 `6,688B`，本阶段只记录，不转入包体优化。
...
49. 实现已完成，等待产品验收：M10-A 已沉淀单角色 Discovery/Normalize/Compile/Validate/capture/apply 流水线和小玉金标准，单 owner 与全量审计幂等，20 角色队列已生成。完整单测与 M10 相关 production 路径通过；全历史 E2E 的旧按需面板定位失败及总 JS gzip `746,688B` 单独记录，不在本阶段修性能、重写旧测试或压包。M10-B 不自动启动。
