...
当前里程碑：**M4 / 约束感知排轴与自动整理实施中，M4-A 已完成**。统一放置提议现已复用 compiler、execution plan、action rule diagnostics 与受控角色时间线，为动作库插入、已有动作移动、复制粘贴和批次平移生成 `valid / adjustable / blocked / unresolved` 结果；自由排轴仍保持原提交语义，下一步进入 M4-B，把用户明确选择的约束辅助接成单次历史事务。未知条件只提示，不自动推断。M4 不接入新战斗公式、真实采样、培养效果或分析报告。
...
27. 实施中：M4-A 统一合法放置提议已完成，动作插入、移动、复制粘贴与批次平移共享现有占用/CD/受控角色/关系/边界评估；下一步完成 M4-B 自由/约束辅助分段模式和单次历史事务，再进入 M4-C 同轴反馈与真实工作流验收。
